  ...
  try
    ...
    begin
      ...
      or (PropInfo^.PropType^.Kind in [tkLString,
        {$IFNDEF COMPILER3_UP}tkLString, {$ENDIF}tkWChar])
        {$ENDIF WIN32}
      then
...
begin
  ...
  begin
    ...
    if (S <> '') or (PropInfo^.PropType^.Kind in [tkString
      {$IFDEF WIN32}, tkLString, {$IFNDEF COMPILER3_UP} tkLString, {$ENDIF}
...
begin
  ...
  //TypeInfo := GetTypeData(PTypeInfo(GetPropType(PropInfo)))^.CompType{$IFDEF COMPILER3_UP}^{$ENDIF};
