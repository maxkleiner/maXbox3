...
type
  ...
  IAsyncCall = interface
   ['{AAAAD8EE-17B6-4FC7-AC94-77887201EEEE}']
...
function AsyncCallX(Proc: TAsyncCallArgFunction; Arg: Integer): IAsyncCall;// overload;
...
//function AsyncCallX(Proc: TAsyncCallArgIntegerProc; Arg: Integer): Integer;
function AsyncCall(Proc: TAsyncCallArgIntegerProc; Arg: Integer): IAsyncCall;
begin
    //result:=30;
    //  result:= TAsyncCall.Create(TInternalasynccall.Create);
    //proc:= @sleep(200);
    //proc:= NIL;
    result:= AsyncCall(TAsyncCallArgObjectProc(Proc), TObject(Arg));

  //Result:= AsyncCall(TAsyncCallArgIntegerProc(Proc), TObject(Arg));
end;

function AsyncCallX(Proc: TAsyncCallArgFunction; Arg: Integer): IAsyncCall;
//function AsyncCall(Proc: TAsyncCallArgIntegerProc; Arg: Integer): IAsyncCall;
begin
    //result:=30;
    //  result:= TAsyncCall.Create(TInternalasynccall.Create);
    //proc:= @sleep(200);
    //proc:= NIL;
    result:= AsyncCall(TAsyncCallArgIntegerProc(Proc), Arg);
