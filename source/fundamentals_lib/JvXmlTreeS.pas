...
// Test with TStrings instead of TList
...
type
  ...
  private
    ...
    FParent: TJvXMLNodeS;
  public
    constructor Create(AParent: TJVXMLNodeS; const AName: string; AValue: Variant);
    function Document: string;
    property Name: string read FName write FName;
    property Value: Variant read FValue write FValue;
    property Parent: TJVXMLNodeS read FParent write FParent;
  end;

  TJvXMLNodeS = class(TObject)
  private
    FName: string;
    FValue: Variant;
    FNodes: TStringList;
    FAttributes: TStringList;
    FParentNode: TJvXMLNodeS;
    FValueType: TJvXMLValueType;
  public
    constructor Create(const AName: string; AValue: Variant; AParent: TJVXMLNodeS);
    destructor Destroy; override;
    // added 29-July-2000
    function GetNamePathNode(const APath: string): TJVXMLNodeS;
    procedure DeleteNamePathNode(const APath: string);
    function ForceNamePathNode(const APath: string): TJVXMLNodeS;
    ...
    function AddNode(const AName: string; AValue: Variant): TJVXMLNodeS;
    function AddNodeEx(const AName: string; AValue: Variant): TJVXMLNodeS;
    ...
    function GetNamedNode(const AName: string): TJVXMLNodeS;
    function SelectSingleNode(const APattern: string): TJVXMLNodeS;
    procedure SelectNodes(APattern: string; AList: TList);
    function TransformNode(AStyleSheet: TJVXMLNodeS): string;
    function Process(ALevel: Integer; ANode: TJVXMLNodeS): string;
    function FindNamedNode(const AName: string): TJVXMLNodeS;
    ...
    function CloneNode: TJVXMLNodeS;
    function FirstChild: TJVXMLNodeS;
    function LastChild: TJVXMLNodeS;
    function PreviousSibling: TJVXMLNodeS;
    function NextSibling: TJVXMLNodeS;
    function MoveAddNode(Dest: TJVXMLNodeS): TJVXMLNodeS;
    function MoveInsertNode(Dest: TJVXMLNodeS): TJVXMLNodeS;
    function RemoveChildNode(ANode: TJVXMLNodeS): TJVXMLNodeS;
    property Name: string read FName write FName;
    property Value: Variant read FValue write FValue;
    property ValueType: TJvXMLValueType read FValueType write FValueType;
    property Nodes: TStringList read FNodes write FNodes;
    property ParentNode: TJVXMLNodeS read FParentNode write FParentNode;
    property Attributes: TStringList read FAttributes write FAttributes;
  end;

  TJvXMLTreeS = class(TJVXMLNodeS)
  private
    FLines: TStringList;
    FNodeCount: Integer;
...
const
  UnitVersioning: TUnitVersionInfo = (
    RCSfile: '$URL: https://jvcl.svn.sourceforge.net/svnroot/jvcl/branches/JVCL3_40_PREPARATION/run/JvXmlTree.pas $';
...
constructor TJVXMLNodeS.Create(const AName: string; AValue: Variant; AParent: TJVXMLNodeS);
begin
  inherited Create;
...
destructor TJVXMLNodeS.Destroy;
begin
  ClearNodes;
  FNodes.Free;
  ClearAttributes;
...
function TJVXMLNodeS.AddAttribute(const AName: string; AValue: Variant): TJvXMLAttribute;
begin
  Result := TJvXMLAttribute.Create(Self, AName, AValue);
  Attributes.AddObject('TJvXMLAttribute',Result);
end;

function TJVXMLNodeS.AddNode(const AName: string; AValue: Variant): TJVXMLNodeS;
begin
  Result := TJVXMLNodeS.Create(AName, AValue, Self);
  Nodes.AddObject('TJVXMLNodeS',Result);
end;

// adds node and parses any attributes;

function TJVXMLNodeS.AddNodeEx(const AName: string; AValue: Variant): TJVXMLNodeS;
var
  S, SN, SV: string;
  C, P1, P2: Integer;
begin
  Result := TJVXMLNodeS.Create(AName, AValue, Self);
  Self.Nodes.AddObject('TJVXMLNodeS',Result);
  C := Length(AName);
  //first parse Name
  P1 := PosStr(' ', AName, 1);
  if P1 = 0 then
...
function TJVXMLNodeS.GetNamedAttribute(const AName: string): TJvXMLAttribute;
var
  I: Integer;
  N: TJvXMLAttribute;
begin
...
procedure TJVXMLNodeS.ClearAttributes;
var
  I: Integer;
begin
...
procedure TJVXMLNodeS.ClearNodes;
var
  I: Integer;
begin
  for I := 0 to Nodes.Count - 1 do
    TJVXMLNodeS(Nodes[I]).Free;
  Nodes.Clear;
end;

procedure TJVXMLNodeS.DeleteAttribute(Index: Integer);
begin
...
procedure TJVXMLNodeS.DeleteNode(Index: Integer);
begin
  if (Nodes.Count > 0) and (Index < Nodes.Count) then
  begin
    TJVXMLNodeS(Nodes[Index]).Free;
    Nodes.Delete(Index);
  end;
end;

function TJVXMLNodeS.Document(ALevel: Integer): string;
var
  I: Integer;
  Indent: string;
...
begin
  ...
  if Nodes.Count <> 0 then
    for I := 0 to Nodes.Count - 1 do
      Result := Result + TJVXMLNodeS(Nodes[I]).Document(ALevel + 1);
  Result := Result + Indent + '</' + Name + '>' + sLineBreak;
end;

// duplicates a node recursively

function TJVXMLNodeS.CloneNode: TJVXMLNodeS;
var
  I: Integer;
  N: TJVXMLNodeS;
begin
  Result := TJVXMLNodeS.Create(Name, Value, nil);
  Result.Name := Name;
  Result.Value := Value;
  if Attributes.Count > 0 then
    for I := 0 to Attributes.Count - 1 do
  ...
  if Nodes.Count > 0 then
    for I := 0 to Nodes.Count - 1 do
    begin
      N := TJVXMLNodeS(Nodes[I]).CloneNode;
      Result.Nodes.AddObject('TJVXMLNodeS',N);
    end;
end;

function TJVXMLNodeS.GetNamedNode(const AName: string): TJVXMLNodeS;
var
  I: Integer;
  N: TJVXMLNodeS;
begin
...
function TJVXMLNodeS.FirstChild: TJVXMLNodeS;
begin
  if Nodes.Count > 0 then
    Result := TJVXMLNodeS(Nodes[0])
  else
    Result := nil;
end;

function TJVXMLNodeS.LastChild: TJVXMLNodeS;
begin
  if Nodes.Count > 0 then
    Result := TJVXMLNodeS(Nodes[Nodes.Count - 1])
  else
    Result := nil;
end;

function TJVXMLNodeS.NextSibling: TJVXMLNodeS;
var
  Index: Integer;
begin
  Result := nil;
  if ParentNode = nil then
    Exit;
  Index := ParentNode.Nodes.IndexOf('TJVXMLNodeS');
  if Index = -1 then
    Exit;
  if Index < ParentNode.Nodes.Count - 1 then
    Result := TJVXMLNodeS(ParentNode.Nodes[Index + 1]);
end;

function TJVXMLNodeS.PreviousSibling: TJVXMLNodeS;
var
  Index: Integer;
begin
  Result := nil;
...
function TJVXMLNodeS.MoveInsertNode(Dest: TJVXMLNodeS): TJVXMLNodeS;
var
...
begin
  ...
  Index1 := Self.ParentNode.Nodes.IndexOf('TJVXMLNodeS');
  if Index1 = -1 then
    Exit;
  Index2 := Dest.ParentNode.Nodes.IndexOf('TJVXMLNodeS');
  if Index2 = -1 then
    Exit;
  Dest.ParentNode.Nodes.InsertObject(Index2, 'TJVXMLNodeS',Self);
  Self.ParentNode.Nodes.Delete(Index1);
  Self.ParentNode := Dest.ParentNode;
  Result := Self;
end;

function TJVXMLNodeS.MoveAddNode(Dest: TJVXMLNodeS): TJVXMLNodeS;
var
  Index: Integer;
begin
  Result := nil;
  if Dest = nil then
    Exit; // can not move to root
  Index := Self.ParentNode.Nodes.IndexOf('TJVXMLNodeS');
  if Index = -1 then
    Exit;
  Dest.Nodes.AddObject('TJVXMLNodeS',Self);
...
function TJVXMLNodeS.RemoveChildNode(ANode: TJVXMLNodeS): TJVXMLNodeS;
var
  Index: Integer;
begin
  Result := nil;
...
function TJVXMLNodeS.HasChildNodes: Boolean;
begin
  Result := Nodes.Count > 0;
end;

procedure TJVXMLNodeS.GetAttributeNames(AList: TStrings);
var
  I: Integer;
begin
  AList.Clear;
...
procedure TJVXMLNodeS.GetNodeNames(AList: TStrings);
var
  I, C: Integer;
begin
  AList.Clear;
  C := Nodes.Count;
  for I := 0 to C - 1 do
    AList.Add(TJVXMLNodeS(Nodes[I]).Name);
end;

function TJVXMLNodeS.GetNodePath: string;
var
  N: TJVXMLNodeS;
begin
  N := Self;
...
function TJVXMLNodeS.FindNamedNode(const AName: string): TJVXMLNodeS;
var
  I: Integer;
  N: TJVXMLNodeS;
begin
...
procedure TJVXMLNodeS.FindNamedNodes(const AName: string; AList: TList);
var
  I: Integer;
  N: TJVXMLNodeS;
begin
...
// typecast to use, e.g. N:=TJVXMLNodeS(AList[0]);

procedure TJVXMLNodeS.GetAllNodes(AList: TList);
var
  I: Integer;
  N: TJVXMLNodeS;
begin
...
procedure TJVXMLNodeS.FindNamedAttributes(const AName: string; AList: TList);
var
  I: Integer;
begin
  for I := 0 to Attributes.Count - 1 do
  ...
  for I := 0 to Nodes.Count - 1 do
    TJVXMLNodeS(Nodes[I]).FindNamedAttributes(AName, AList);
end;
...
procedure TJVXMLNodeS.MatchPattern(const APattern: string; AList: TList);
begin
  // to be implemented
end;
...
function TJVXMLNodeS.SelectSingleNode(const APattern: string): TJVXMLNodeS;
var
...
function TJVXMLNodeS.MatchFilter(AObjFilter: TJvXMLFilter): Boolean;
var
  I, J: Integer;
  A: TJvXMLAttribute;
  N: TJVXMLNodeS;
...
procedure TJVXMLNodeS.SelectNodes(APattern: string; AList: TList);
var
  NPattern: string;
  P, I: Integer;
  N: TJVXMLNodeS;
...
function TJVXMLNodeS.TransformNode(AStyleSheet: TJVXMLNodeS): string;
begin
  // to be implemented;
...
function TJVXMLNodeS.Process(ALevel: Integer; ANode: TJVXMLNodeS): string;
var
  I: Integer;
  Indent: string;
...
begin
  ...
  begin
    for I := 0 to Nodes.Count - 1 do
      Result := Result + TJVXMLNodeS(Nodes[I]).Process(ALevel + 1, ANode);
    Exit;
  end;
  if ALevel > 0 then
    Indent := StringOfChar(' ', ALevel * 2)
...
function TJVXMLNodeS.GetNameSpace: string;
var
  P: Integer;
begin
  P := Pos(':', FName);
...
function TJVXMLNodeS.GetNamePathNode(const APath: string): TJVXMLNodeS;
var
...
function TJVXMLNodeS.ForceNamePathNode(const APath: string): TJVXMLNodeS;
var
...
begin
  ...
  N := TJVXMLNodeS.Create(AName, '', Self);
  Nodes.Add(N);
  if NewPath = '' then
    Result := N
  else
...
function TJVXMLNodeS.ForceNamePathNodeAttribute(const APath, AName: string;
...
function TJVXMLNodeS.GetNamePathNodeAttribute(const APath, AName: string): TJvXMLAttribute;
var
  N: TJVXMLNodeS;
begin
  Result := nil;
...
procedure TJVXMLNodeS.DeleteNamePathNode(const APath: string);
var
  N, PN: TJVXMLNodeS;
  I: Integer;
begin
  ...
  for I := 0 to PN.Nodes.Count - 1 do
    if TJVXMLNodeS(PN.Nodes[I]) = N then
    begin
      PN.DeleteNode(I);
      Exit;
    end;
...
procedure TJVXMLNodeS.DeleteNamePathNodeAttribute(const APath, AName: string);
var
  A: TJvXMLAttribute;
  PN: TJVXMLNodeS;
  I: Integer;
...
function TJVXMLNodeS.GetAttributeValue(const AName: string): Variant;
var
  I: Integer;
  A: TJvXMLAttribute;
begin
...
constructor TJvXMLTree.Create(const AName: string; AValue: Variant; AParent: TJVXMLNodeS);
begin
...
begin
  ...
  Result := Result + '>' + sLineBreak;
  for I := 0 to Nodes.Count - 1 do
    Result := Result + TJVXMLNodeS(Nodes[I]).Document(1);
...
begin
  ...
  Result := '';
  for I := 0 to Nodes.Count - 1 do
    Result := Result + TJVXMLNodeS(Nodes[I]).Document(0);
  //  Result:=Result+'</'+Name+'>'+sLineBreak;
end;
...
constructor TJvXMLAttribute.Create(AParent: TJVXMLNodeS; const AName: string; AValue: Variant);
begin
  inherited Create;
  FName := AName;
  FValue := AValue;
