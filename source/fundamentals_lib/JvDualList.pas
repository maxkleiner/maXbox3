...
// enhanced with size properties of delegate form.listbox, max
...
type
  ...
  private
    ...
    procedure SetListboxheight(const Value: integer);
  protected
  ...
  published
    ...
    property Listboxheight: integer read FListboxheight write SetListboxheight;
    ...
    property listboxscrollwidth: integer
             read FlistboxScrollwidth write Flistboxscrollwidth;
...
uses
  Consts, SysUtils,
  JvDualListForm, JvResources; //, JvVCLUtils;//, JvJVCLUtils;
...
procedure TJvDualListDialog.SetListboxheight(const Value: integer);
begin
  FListboxheight := Value;
end;
...
begin
  ...
  try
    ...
    //Form.SrcList.scrollbars:= ssboth;
    if Flistboxheight <> 0 then begin
      Form.SrcList.height:= FListboxheight;
      Form.DstList.height:= FListboxheight;
    end;
    if Flistboxscrollwidth <> 0 then begin
      Form.SrcList.ScrollWidth:= FListboxscrollwidth;
    end;
    ...
    if CenterOnControl <> nil then begin
