...
begin
    Result:= 'PascalScript maXbox3 - RemObjects & SynEdit';
end;
  ...
  begin
    ...
    for l := 0 to HDR.ProcCount - 1 do begin
      ...
      begin
        ...
        begin
          if (not Read(L2, 4)) or (L2 > Length(s) - Pos) then begin
            Curr.Free;
            cmd_err(erUnexpectedEof);
            LoadProcs := False;
            exit;
      ...
      if (Rec.Flags and 4) <> 0 then begin
        if not ReadAttributes(Curr.Attributes) then begin
          Curr.Free;
          LoadProcs := False;
          exit;
        end;
...
{$WARNINGS ON}
  ...
  begin
    ...
    for l := 0 to HDR.VarCount - 1 do begin
      ...
      if (Rec.Flags and 1) <> 0 then begin
        ...
        try
          SetLength(e^.FName, n);
          if not Read(e^.FName[1], n) then begin
            dispose(e);
            cmd_err(erUnexpectedEof);
            LoadVars := False;
            exit;
...
begin
  ...
  Pos := 0;
  LoadData := False;
  if not read(HDR, SizeOf(HDR)) then begin
    CMD_Err(erInvalidHeader);
    exit;
  end;
  if HDR.HDR <> PSValidHeader then begin
    CMD_Err(erInvalidHeader);
    exit;
  end;
  ...
  if not LoadTypes then begin
    Clear;
    exit;
  end;
  if not LoadProcs then begin
    Clear;
    exit;
  end;
  if not LoadVars then begin
    Clear;
    exit;
  end;
...
procedure VNSetPointerTo(const Src: TPSVariantIFC; Data: Pointer; aType: TPSTypeRec);
begin
  if Src.aType.BaseType = btPointer then begin
    if atype.BaseType in NeedFinalization then
...
begin
  if FCurrentPosition + 4 >= FDataLength then begin
  ...
  case VarType of
    0:
      begin
        ...
        if Param < PSAddrNegativeStackStart then begin
          if Param >= Cardinal(FGlobalVars.Count) then begin
          ...
          if Param >= Cardinal(FStack.Count) then begin
            CMD_Err(erOutOfGlobalVarsRange);
            Result := False;
            exit;
          end;
        ...
        if (UsePointer) and (Tmp.FType.BaseType = btPointer) then begin
          Dest.aType := PPSVariantPointer(Tmp).DestType;
          Dest.P := PPSVariantPointer(Tmp).DataDest;
          if Dest.P = nil then begin
      ...
      {$ENDIF}
        ...
        case Dest.aType.BaseType of
          ...
          bts16, {$IFNDEF PS_NOWIDESTRING}btwidechar,{$ENDIF} btU16:
            begin
              if FCurrentPosition + 1>= FDataLength then begin
                CMD_Err(erOutOfRange);
                FTempVars.Pop;
                Result := False;
                exit;
          ...
          bts64:
            begin
              if FCurrentPosition + 7>= FDataLength then begin
                CMD_Err(erOutOfRange);
                FTempVars.Pop;
                Result := False;
                exit;
          ...
          btSingle:
            begin
              if FCurrentPosition + (Sizeof(Single)-1)>= FDataLength then begin
                CMD_Err(erOutOfRange);
                FTempVars.Pop;
                Result := False;
                exit;
          ...
          btDouble:
            begin
              if FCurrentPosition + (Sizeof(Double)-1)>= FDataLength then begin
                CMD_Err(erOutOfRange);
                FTempVars.Pop;
                Result := False;
                exit;
          ...
          btExtended:
            begin
              if FCurrentPosition + (sizeof(Extended)-1)>= FDataLength then begin
                CMD_Err(erOutOfRange);
                FTempVars.Pop;
                Result := False;
                exit;
	      ...
	      {$endif}
              ...
              if not ReadData(tbtstring(Dest.P^)[1], Param) then begin
                CMD_Err(erOutOfRange);
                FTempVars.Pop;
                Result := False;
                exit;
	      ...
	      {$endif}
              ...
              if not ReadData(tbtwidestring(Dest.P^)[1], Param*2) then begin
                CMD_Err(erOutOfRange);
                FTempVars.Pop;
                Result := False;
                exit;
	      ...
	      {$endif}
              ...
              if not ReadData(tbtUnicodestring(Dest.P^)[1], Param*2) then begin
                CMD_Err(erOutOfRange);
                FTempVars.Pop;
                Result := False;
                exit;
    ...
    2:
      begin
        ...
        if Tmp.FType.BaseType = btPointer then begin
          Dest.aType := PPSVariantPointer(Tmp).DestType;
          Dest.P := PPSVariantPointer(Tmp).DataDest;
          if Dest.P = nil then begin
            Cmd_Err(erNullPointerException);
	...
	{$endif}
        ...
        begin
          ...
          Param := Cardinal(Longint(-PSAddrStackStart) + Longint(FCurrStackBase) + Longint(Param));
          if Cardinal(Param) >= Cardinal(FStack.Count) then begin
            CMD_Err(erOutOfStackRange);
            Result := false;
            exit;
          end;
        ...
        case Tmp.FType.BaseType of
          ...
          btPointer:
            begin
              if PPSVariantPointer(tmp).DestType <> nil then begin
        ...
        case Dest.aType.BaseType of
          ...
          btStaticArray:
            begin
              if Param >= Cardinal(TPSTypeRec_StaticArray(Dest.aType).Size) then begin
                CMD_Err(erOutOfRange);
                Result := False;
                exit;
              end;
...
begin
  ...
  if FStatus = isLoaded then
  begin
    for i := FExceptionStack.Count -1 downto 0 do begin
      pp := FExceptionStack.Data[i];
      pp.Free;
    end;
    FExceptionStack.Clear;
  ...
  case FStatus of
    isLoaded: begin
        if FMainProc = InvalidVal then begin
          RunScript := False;
          exit;
        end;
        FStatus := isRunning;
          ...
          CM_A:
            begin
              if not ReadVariable(vd, True) then
                break;
              if vd.FreeType <> vtNone then begin
              ...
              if not ReadVariable(vs, True) then Break;
              // nx change end
{              if (vd.aType.BaseType = btClass) and (vs.aType.BaseType in [btS32]) then
              ...
              // nx change start
              if not SetVariantValue(vd.P, vs.P, vd.aType, vs.aType) then begin
                if vs.FreeType <> vtNone then begin
          ...
          CM_CA:
            begin
              ...
              if not ReadVariable(vs, True) then
                Break;
              if not DoCalc(vd.P, vs.p, vd.aType, vs.aType, CalcType) then begin
                if vs.FreeType <> vtNone then begin
                  if vs.aType.BaseType in NeedFinalization then
                  FinalizeVariant(vs.P, vs.aType);
                  p := IPointer(FTempVars.Data^[FtempVars.Count-1]) - IPointer(FtempVars.DataPtr);
          ...
          CM_PV:
            begin
              ...
              if vs.aType.BaseType = btPointer then begin
          ...
          CM_PO: begin
              ...
              if (vtemp = nil) or (vtemp.FType.BaseType = btReturnAddress) then begin
                CMD_Err(erOutOfStackRange);
                break;
              end;
              FStack.Pop;
(*              Dec(FStack.FCount);
              ...
              if u.ClassType = TPSExternalProcRec then begin
                try
                  ...
                  if Tmp <> nil then begin
                    ...
                    if Tmp is EDivByZero then begin
                      CMD_Err3(erDivideByZero, tbtString(Exception(Tmp).Message), Tmp);
                      Break;
                    end;
          ...
          Cm_R: begin
              ...
              if FExceptionStack.Count > 0 then begin
                ...
                begin
                  if pp.StackSize < Cardinal(FStack.Count) then begin
                    for p := Longint(FStack.count) -1 downto Longint(pp.StackSize) do
                      FStack.Pop
                  end;
                  FCurrStackBase := pp.BasePtr;
                  if pp.FinallyOffset <> InvalidVal then begin
                    FCurrentPosition := pp.FinallyOffset;
                    pp.FinallyOffset := InvalidVal;
                    p2 := 1;
                    break;
                  end else if pp.Finally2Offset <> InvalidVal then begin
                    FCurrentPosition := pp.Finally2Offset;
                    pp.Finally2Offset := InvalidVal;
                    p2 := 1;
                    break;
              ...
              if p2 = 0 then begin
                FExitPoint := InvalidVal;
                if FCurrStackBase = InvalidVal then begin
                  FStatus := FOldStatus;
                  break;
                end;
                for P2 := FStack.Count - 1 downto FCurrStackBase + 1 do
                  FStack.Pop;
                if FCurrStackBase >= FStack.Count then begin
                  FStatus := FOldStatus;
                  break;
                end;
          ...
          cm_puexh:
            begin
              ...
              if ((pp.FinallyOffset <> InvalidVal) and (pp.FinallyOffset >= FDataLength)) or
                ...
                ((pp.EndOfBlock <> InvalidVal) and (pp.EndOfBlock >= FDataLength)) then begin
                  CMD_Err(ErOutOfRange);
                  pp.Free;
                  Break;
                end;
              ...
              case p of
                2:
                  begin
                    ...
                    if pp.Finally2Offset <> InvalidVal then begin
                      FCurrentPosition := pp.Finally2Offset;
                      pp.Finally2Offset := InvalidVal;
                    end else begin
                      p := pp.EndOfBlock;
                      pp.Free;
                      FExceptionStack.DeleteLast;
                      if FExitPoint <> InvalidVal then begin
                ...
                1:
                  begin
                    ...
                    begin
                      ...
                      pp.Free;
                      FExceptionStack.DeleteLast;
                      if (ExEx <> eNoError) and (p <> InvalidVal) then begin
                        Tmp := ExObject;
                        ExObject := nil;
                        ExceptionProc(ExProc, ExPos, ExEx, ExParam, Tmp);
                      end else
          ...
          cm_spc:
            begin
              ...
              if (Vd.aType.BaseType <> btPointer) then begin
                CMD_Err(erInvalidOpcodeParameter);
                break;
              end;
              ...
              if vs.aType.BaseType = btPointer then begin
                if Pointer(vs.P^) <> nil then begin
                ...
                if not CopyArrayContents(Pointer(vd.P^), vs.P, 1, vs.aType) then begin
                  if vs.FreeType <> vtNone then
                    FTempVars.Pop;
                  CMD_Err(ErTypeMismatch);
                  break;
...
{$ENDIF}
          ...
          Cm_cv:
            begin
              if not ReadVariable(vd, True) then
                Break;
              if vd.aType.BaseType <> btProcPtr then begin
                if vd.FreeType <> vtNone then
                  FTempVars.Pop;
                CMD_Err(ErTypeMismatch);
                break;
          ...
          CM_CO:
            begin
              ...
              if not ReadVariable(v3, True) then
                Break;
              if v3.FreeType <> vtNone then begin
              ...
              if not ReadVariable(vs, True) then
                Break;
              if not ReadVariable(vd, True) then begin
                if vs.FreeType <> vtNone then begin
                  if vs.aType.BaseType in NeedFinalization then
                    FinalizeVariant(vs.P, vs.aType);
                  p := IPointer(FTempVars.Data^[FtempVars.Count-1]) - IPointer(FtempVars.DataPtr);
...
begin
  case Longint(p.Ext1) of
    //by max
     51: Stack.SetString(-1, InttoHex(Stack.GetInt(-2), Stack.GetInt(-3)));
     {$C+}                                                //bugfix 3.9.6
    52: Assert(Stack.GetBool(-1), Stack.GetString(-2));
    {$C-}
    //53: Stack.SetReal(-1, Power(Stack.GetReal(-2),Stack.GetReal(-3)));
    54: Stack.SetReal(-1, Frac(Stack.GetReal(-2)));
    //55: Stack.SetReal(-1, LogN(Stack.GetReal(-2),Stack.GetReal(-3)));
    //function
    //56: Stack.SetInt(-1, HexToBin(pchar(Stack.getString(-2)),
      //                     pchar(Stack.getString(-3)), stack.getInt(-3)));
    57: Stack.SetReal(-1, Ln(Stack.GetReal(-2)));
    //58: Stack.SetString(-1, DateTimeToStr(Stack.GetReal(-2)));
    59: Stack.SetReal(-1, SQR(Stack.GetReal(-2))); // Sqr
    60: Stack.SetReal(-1, Exp(Stack.GetReal(-2)));
    61: Stack.SetBool(-1, Odd(Stack.GetInt(-2)));
...
begin
  ...
  RegisterFunctionName('ARRAYLENGTH', GetArrayLength, nil, nil);
  ...
  //by max
  RegisterFunctionName('INTTOHEX', DefProc, Pointer(51), nil);
  RegisterFunctionName('ASSERT', DefProc, Pointer(52), NIL);
  //RegisterFunctionName('POWER', DefProc, Pointer(53), NIL);
  RegisterFunctionName('FRAC', DefProc, Pointer(54), NIL);
  //RegisterFunctionName('LOGN', DefProc, Pointer(55), NIL); //of math
  //RegisterFunctionName('INC', DefProc, Pointer(52), nil);
  //RegisterFunctionName('DEC', DefProc, Pointer(53), nil);
  //RegisterFunctionName('HEXTOBIN', DefProc, Pointer(56), NIL);
  RegisterFunctionName('LN', DefProc, Pointer(57), NIL);
  RegisterFunctionName('SQR', DefProc, Pointer(59), NIL);
  RegisterFunctionName('EXP', DefProc, Pointer(60), NIL);
  RegisterFunctionName('ODD', DefProc, Pointer(61), NIL);
  //RegisterFunctionName('BINTOHEX', DefProc, Pointer(62), NIL);

  //RegisterFunctionName('DATETIMETOSTR', DefProc, Pointer(58), NIL);
