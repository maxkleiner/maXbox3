...
{$ENDIF}
procedure RIRegister_TMenuItemEnumerator(CL: TPSRuntimeClassImporter);
procedure RIRegister_TMenuActionLink(CL: TPSRuntimeClassImporter);
...
{$IFNDEF FPC}
 ...
 S.RegisterDelphiFunction(@ShortCutFromMessage, 'ShortCutFromMessage', cdRegister);
 ...
 S.RegisterDelphiFunction(@IsAltGRPressed, 'IsAltGRPressed', cdRegister);
...
begin
	with Cl.Add(TMENU) do begin
		...
		RegisterMethod(@TMENU.CreateMenuItem, 'CreateMenuItem');
...
begin
	...
	begin
        ...
        RegisterMethod(@TMENUITEM.Destroy, 'Free');
...
(*----------------------------------------------------------------------------*)
procedure TMenuItemEnumeratorCurrent_R(Self: TMenuItemEnumerator; var T: TMenuItem);
...
procedure RIRegister_TMenuItemEnumerator(CL: TPSRuntimeClassImporter);
begin
  with CL.Add(TMenuItemEnumerator) do begin
    RegisterConstructor(@TMenuItemEnumerator.Create, 'Create');
    RegisterMethod(@TMenuItemEnumerator.GetCurrent, 'GetCurrent');
    RegisterMethod(@TMenuItemEnumerator.MoveNext, 'MoveNext');
    RegisterPropertyHelper(@TMenuItemEnumeratorCurrent_R,nil,'Current');
  end;
end;
...
procedure RIRegister_TMenuActionLink(CL: TPSRuntimeClassImporter);
begin
  with CL.Add(TMenuActionLink) do
  begin
  end;
end;
...
begin
	...
	RIRegisterTMENU(Cl);
 RIRegister_TMenuActionLink(CL);
  RIRegister_TMenuItemEnumerator(CL);
