...
uses Clipbrd;
procedure TvtMainForm.ConnButtonClick(Sender: TObject);
begin
  ComTerminal.Connected := not ComTerminal.Connected;
end;

procedure TvtMainForm.ComPortAfterOpen(Sender: TObject);
begin
  ConnButton.Caption := 'Disconnect';
end;

procedure TvtMainForm.ComDataPacket1CustomStart(Sender: TObject;
...
procedure TvtMainForm.ComDataPacket1CustomStop(Sender: TObject; const Str: string;
                                                            var Pos: Integer);
begin
  if Pos >=0 then
...
procedure TvtMainForm.ComDataPacket1Packet(Sender: TObject; const Str: string);
begin
  StatusBar1.Panels[0].Text := 'FOUND: '+str+  '                                  '  ;
  ComDataPacket1.ResetBuffer;
end;

procedure TvtMainForm.ComPortAfterClose(Sender: TObject);
begin
  ConnButton.Caption := 'Connect';
end;

procedure TvtMainForm.Paste1Click(Sender: TObject);
var s:string;
begin
  s  := Clipboard.AsText;
  ComPort.WriteStr(s);
...
procedure TvtMainForm.PortButtonClick(Sender: TObject);
begin
  ComPort.ShowSetupDialog;
end;

procedure TvtMainForm.TermButtonClick(Sender: TObject);
begin
  ComTerminal.ShowSetupDialog;
end;

procedure TvtMainForm.FontButtonClick(Sender: TObject);
begin
  ComTerminal.SelectFont;
end;

procedure TvtMainForm.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TvtMainForm.FormShow(Sender: TObject);
begin
...
procedure TvtMainForm.Button1Click(Sender: TObject);
var  s:String;
begin
