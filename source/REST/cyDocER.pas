{ cyDER  Components
  ...
  Todo: 2013/08/05 - Do not merge expressions if one ot them and not the other belongs to a element defined TRect pattern
}
...
uses
  Classes, Windows, Controls, SysUtils, cyStrUtils, cyDERUtils, cyGraphics, cyDateUtils;
...
type
  ...
  TcyElement = class(TCollectionItem)
  private
    FTag: Integer;
    FKeyWords: TStrings;
  ...
  TElementClass = class of TcyElement;

  TElements = Class(TCollection)
  private
    FOwner: TPersistent;
    FOnChange: TNotifyEvent;
    function GetElement(Index: Integer): TcyElement;
  protected
    function GetOwner: TPersistent; Override;
    procedure Update(Item: TCollectionItem); Override;
  public
    constructor Create(aDocument: TcyDocER; ElementClass: TElementClass);
    function Add: TcyElement;
    procedure Delete(Index: Integer);
    property Items[Index: Integer]: TcyElement read GetElement; default;
  ...
  protected
    ...
    function DoStringToDERCharSet(aString: String): String;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    // OCR feeding :
    procedure NewDocument(ImagePageCount, ImageResolution, DefaultImagePixelsWidth, DefaultImagePixelsHeight: Integer);
    ...
    function CloneExpression(aExpressionIndex: Integer): Integer;
    ...
    function IsExpressionsSameValue(ExpressionIndex1, ExpressionIndex2: Integer): Boolean;
...
{ TElement }
constructor TcyElement.Create(Collection: TCollection);
begin
...
destructor TcyElement.Destroy;
begin
  FKeyWords.Free;
  inherited;
end;

function TcyElement.GetDisplayName: string;
var k: Integer;
begin
  Result := '';
...
function TcyElement.PatternRecognition: boolean;
begin
  Result := (FPatternFromLeftMm <> 0) or (FPatternFromTopMm <> 0) or (FPatternToRightMm <> 0) or (FPatternToBottomMm <> 0);
end;

procedure TcyElement.SetKeyWords(const Value: TStrings);
begin
...
procedure TcyElement.Assign(Source: TPersistent);
begin
  if Source is TcyElement then
  begin
    FKeyWords  := TcyElement(Source).FKeyWords;
    FPatternPageNumber := TcyElement(Source).FPatternPageNumber;
    FPatternPageNumberMode := TcyElement(Source).FPatternPageNumberMode;
    FPatternFromLeftMm := TcyElement(Source).FPatternFromLeftMm;
    FPatternFromTopMm := TcyElement(Source).FPatternFromTopMm;
    FPatternToRightMm := TcyElement(Source).FPatternToRightMm;
    FPatternToBottomMm := TcyElement(Source).FPatternToBottomMm;
    FPatternPositionMode := TcyElement(Source).FPatternPositionMode;
    FPatternValueLocation := TcyElement(Source).FPatternValueLocation;
    FPatternValueNumber := TcyElement(Source).FPatternValueNumber;
    FValueCount := TcyElement(Source).FValueCount;
    FValueType := TcyElement(Source).FValueType;
    FTag := TcyElement(Source).FTag;
  end;
...
function TElements.Add: TcyElement;
begin
  Result := TcyElement(inherited Add);
...
function TElements.GetElement(Index: Integer): TcyElement;
begin
  Result := TcyElement(inherited Items[Index]);
end;
...
begin
  ...
  FElements := TElements.Create(self, TcyElement);
  FResolution := 0;
  FPageCount := 0;
  FPixelsHeight := 0;
  FPixelsWidth := 0;
...
function TcyDocER.DoStringToDERCharSet(aString: String): String;
begin
  if eoDERValueAsString in FExpressionOptions then
  begin
    Result := aString;
    Result := StringReplace(Result, ',', '.', [rfReplaceAll]);  // For float recognition ...
  end
  else
    Result := StringToDERCharSet(aString);
end;
...
function TcyDocER.CloneExpression(aExpressionIndex: Integer): Integer;
begin
...
begin
  ...
  begin
    case ValueType of
      etText, etExpressionKeyWord, etTextLine, etParagraph:
      begin
        if AnsiUpperCase(Expressions[i].FDERValue) = AnsiUpperCase(DoStringToDERCharSet(Value)) then Result := i;
      end;
...
begin
  ...
  Value  := DoStringToDERCharSet(Value);
...
procedure TcyDocER.NewDocument(ImagePageCount, ImageResolution, DefaultImagePixelsWidth, DefaultImagePixelsHeight: Integer);
begin
  FPageCount := ImagePageCount;
  FResolution := ImageResolution;
  FPixelsWidth := DefaultImagePixelsWidth;
  FPixelsHeight := DefaultImagePixelsHeight;
end;
  ...
  Locate1stWordOptions, LocateAroundOptions: TLocateExpressionOptions;
      ...
      begin
        ...
        begin
          ...
          if length(DoStringToDERCharSet(RemainedStr)) = LengthRemainedExpression then
          begin
...
function TcyDocER.RecognizeElementValuesFromRect(OfElementIndex, OfElementKeywordIndex, OfExpressionKeywordIndex: Integer; aValueType: TElementsType;
 ...
 ValueNumber, ValueCount: Integer; PatternMode: Boolean; var UserAbort: Boolean): Integer;
       ...
       begin
         ...
         if aValueType in [etText, etTextLine, etParagraph]  // Expression can be recognized as other type
         then
...
begin
      ...
      if Expressions[i].FAssociatedElementIndex <> OfElementIndex then  // Ignore element's Keyword ...
        if (not IsDERSymbols(Expressions[i].FDERValue)) or (SortedExpressionList.Count <> 0) then   // Remove symbol ...
          ...
          begin
            ...
            SortedExpressionList.Add( FormatFloat('0000000', SortValuePosition) + ';' + FormatFloat('0000000', i) );
          end;
  ...
  if PatternMode then
  begin
    // 2013/10/29 Remove all expressions from any already associated to another element :
    for i := ExpressionList.Count-1 downto 0 do
  ...
  // 2013-07-22 Merge expressions if etTextLine or etParagraph :
  if aValueType in [etTextLine, etParagraph] then
  begin
    i := 0;
    ...
    begin
      ...
      if (aValueType = etParagraph) or ExpressionsInSameLine(ExpressionIndex0, ExpressionIndex1) then
      begin
        if aValueType = etParagraph then
        ...
        MergeExpressions(ExpressionIndex1, ExpressionIndex0);
        ExpressionList.Delete(i+1);

        // Resynch:
        for j := 0 to ExpressionList.Count - 1 do
         if StrToInt(ExpressionList[j]) > ExpressionIndex1 then
           ExpressionList[j] := intToStr( StrToInt(ExpressionList[j]) - 1 );
      end
      else
        Break;
    end;
...
begin
  ...
  // Added on 2013-07-19 :
  pxTolerence1 := Round((Expressions[ExpressionCount-1].FBottomPxPos - Expressions[ExpressionCount-1].FTopPxPos) * AutoMergeExpressionsRatio);
  pxTolerence2 := Round((Expressions[ExpressionCount-2].FBottomPxPos - Expressions[ExpressionCount-2].FTopPxPos) * AutoMergeExpressionsRatio);
  ...
  if Expressions[ExpressionCount-2].FDERValue = '-' then
    if Expressions[ExpressionCount-1].FRecognizedType in [etMoney, etFloat, etInteger, etPercentage] then
      if ExpressionsSideBySide(ExpressionCount-2, ExpressionCount-1, pxTolerence) then        // Added on 2014/01/02 ...
      begin
...
begin
  ...
  pxTolerence := PixelsWidth div 5;  // 1/5 sheet width
...
const
  FontSize = 10;
      ...
      begin
        ...
        Result := round( (PixelsHeight / FResolution * FontSize) / AutoMergeExpressionsRatio);
      end;
...
function TcyDocER.IsExpressionsSameValue(ExpressionIndex1, ExpressionIndex2: Integer): Boolean;
begin
  Result := false;
