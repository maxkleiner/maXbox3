...
Type
  ...
  type TALFormatSettings = TFormatSettings;
...
Begin
  ...
  if StatementSQL <> '' then       aWherePart := 'WHERE '+
                                                 ...
                                                 ALIfThen(TransactionID <> -1, ' AND MON$TRANSACTION_ID = '+IntToStr(TransactionID)) +
                                                 ALIfThen(ConnectionID <> -1,  ' AND MON$ATTACHMENT_ID = '+IntToStr(ConnectionID))
...
begin
  ...
  case Item of
    ...
    isc_info_wal_prv_ckpt_fname: begin
                                   aSize := byte(aData[4]);
                                   Move(aData[5], aData[1], aSize);
                                   SetLength(aData, aSize);
                                 end;
    else begin
      aSize := byte(aData[5]);
      Move(aData[6], aData[1], aSize);
      SetLength(aData, aSize);
    end;
  end;
      ...
      while Flibrary.DSQLFetch(fDBHandle, fTraHandle, fStmtHandle, FSQLDIALECT, fsqlda) do begin
        ...
        If aRecIndex > SQLs[aSQLsindex].Skip then begin
          ...
          For aColumnIndex := 0 to fsqlda.FieldCount - 1 do begin
            aValueRec := aNewRec.AddChild(lowercase(fsqlda.AliasName[aColumnIndex]));
            if (fSQLDA.SQLType[aColumnIndex] = SQL_BLOB) then avalueRec.ChildNodes.Add(
                                                 ...
                                                 aColumnIndex,
                                                 FormatSettings);
            if aUpdateRowTagByFieldValue and (aValueRec.NodeName=aNewRec.NodeName) then aNewRec.NodeName := LowerCase(aValueRec.Text);
          end;
...
begin
  ...
  for I := 0 to length(SQLs) - 1 do begin
    ...
    setlength(aUpdateDataSQLs[i].Params,0);
  end;
  UpdateData(aUpdateDataSQLs);
end;
    ...
    while Flibrary.DSQLFetch(aTmpDBHandle, aTmpTraHandle, aTmpStmtHandle, FSQLDIALECT, aTmpSqlda) do begin
      ...
      If aRecIndex > SQLs[aSQLsindex].Skip then begin
        ...
        For aColumnIndex := 0 to aTmpSqlda.FieldCount - 1 do begin
          aValueRec := aNewRec.AddChild(lowercase(aTmpSqlda.AliasName[aColumnIndex]));
  ...
  Try
    ...
    Try
      ...
      For aSQLsindex := 0 to length(SQLs) - 1 do begin
        ...
        if assigned(StatementPool) then aTmpStatementPoolNode := StatementPool.FindNode(SQLs[aSQLsindex].SQL)
        ...
        if not assigned(aTmpStatementPoolNode) then begin
          ...
          if (aReadOnlyReadCommitedTPB) and
            ...
            if length(SQLs[aSQLsindex].Params) > 0 then begin
              ...
              AcquireReadStatement(SQLs[aSQLsindex].SQL,
                                   aTmpDBHandle,
                                   aTmpTraHandle,
                                   aTmpStmtHandle,
                                   aTmpSqlda,
        ...
        try
          ...
          if not assigned(StatementPool) then begin
            ...
            if aStatementWasAcquiredFromPool then begin
              ...
              try
                ...
                ReleaseReadStatement(SQLs[aSQLsindex].SQL,
                                     aTmpDBHandle,
                                     aTmpTraHandle,
                                     aTmpStmtHandle,
                                     aTmpSqlda,
          ...
          {$IFDEF undef}{$REGION 'Else if a statementPool was given in the params'}{$ENDIF}
            ...
            if (length(SQLs[aSQLsindex].Params) > 0) or
              ...
              //else add it to the statement pool
                ...
                aTmpStatementPoolNode.ID := SQLs[aSQLsindex].SQL;
...
begin
  ...
  try
    ...
    For aSQLsindex := 0 to length(SQLs) - 1 do begin
      ...
      {$IFDEF undef}{$REGION 'init aTmpStatementPoolNode'}{$ENDIF}
      if assigned(aTmpStatementPool) then aTmpStatementPoolNode := aTmpStatementPool.FindNode(SQLs[aSQLsindex].SQL)
      else aTmpStatementPoolNode := nil;
      {$IFDEF undef}{$ENDREGION}{$ENDIF}
      ...
      {$IFDEF undef}{$REGION 'if their is NO params and aTmpStatementPoolNode = nil'}{$ENDIF}
        ...
        Flibrary.DSQLExecuteImmediate(aTmpDBHandle, aTmpTraHandle, SQLs[aSQLsindex].SQL, FSQLDIALECT, nil);
...
begin
  ...
  Try
    Alst.Text := ALTrim(StringReplace(aEventNames,';',#13#10,[rfReplaceALL]));
    i := 0;
