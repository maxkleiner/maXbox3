        ...
        if aDataStartPos < 0 then begin
          ...
          try
            ...
            if aResponseType = rpRETRIEVALS then begin
              if (aLst.Count <> 5) or
                 (not TryStrToInt(aLst[2], aflags)) or
                 (not TryStrToInt(aLst[3], aDataLength)) or
                 (not TryStrToInt64(aLst[4], aCasID)) then raise EALException.Create('Memcache Error: Response does not conform to protocol - ' + ALcopyStr(Result, 1, aResultPos));
            end
            else begin
              aCasID := 0;
              if (aLst.Count <> 4) or
                 (not TryStrToInt(aLst[2], aflags)) or
                 (not TryStrToInt(aLst[3], aDataLength)) then raise EALException.Create('Memcache Error: Response does not conform to protocol - ' + ALcopyStr(Result, 1, aResultPos));
            end;
            aKey := aLst[1];
...
begin
  CheckKey(key);
  aResultCode := SendCmd(aSocketDescriptor, 'incr '+Key + ' ' + intToStr(Value), rpINCRDECR);
  if not TryStrToInt64(aResultCode, Result) then raise EAlMemCachedClientException.Create(aResultCode);
end;
...
begin
  CheckKey(key);
  aResultCode := SendCmd(aSocketDescriptor, 'decr '+Key + ' ' + intToStr(Value), rpINCRDECR);
  if not TryStrToInt64(aResultCode, Result) then raise EAlMemCachedClientException.Create(aResultCode);
end;
...
procedure TAlBaseMemCachedClient.DoFlush_all(aSocketDescriptor: TSocket;
                                             delay: integer);
begin
  if delay > 0 then SendCmd(aSocketDescriptor, 'flush_all ' + inttostr(delay), rpOK)
...
Procedure TAlBaseMemCachedClient.DoVerbosity(aSocketDescriptor: TSocket;
                                             level: integer);
begin
  SendCmd(aSocketDescriptor, 'verbosity ' + inttostr(level), rpOK);
end;
