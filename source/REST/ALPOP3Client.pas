...
Begin
  ...
  aResponse := StringReplace(aResponse,#9,' ',[RfReplaceAll]); // +OK 2 320
  While alPos('  ',aResponse) > 0 do aResponse := StringReplace(aResponse,'  ',' ',[RfReplaceAll]); // +OK 2 320
  aResponse := Trim(StringReplace(aResponse,' ',#13#10,[RfReplaceAll]));  // +OK
                                                                              // 2
                                                                              // 320
  aLst.Text := aResponse;
end;
...
begin
  ALst.Text := Trim(StringReplace(ALPOP3ClientExtractTextFromMultilineResponse(List), ' ', ALst.NameValueSeparator, [rfReplaceAll]));
end;
...
begin
  ALst.Text := Trim(StringReplace(ALPOP3ClientExtractTextFromMultilineResponse(Uidl), ' ', ALst.NameValueSeparator, [rfReplaceAll]));
end;
