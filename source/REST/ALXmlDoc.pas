...
begin
  ...
  if SepPos > 0 then begin
    ...
    Move(aName[1], Result[1], SepPos - 1);
  end
  else result := '';
end;
...
begin
  ...
  if SepPos > 0 then begin
    ...
    Move(aName[SepPos + 1], Result[1], aLength); // faster than system.move
  end
  else begin
    aLength := Length(Aname);
    setlength(Result,aLength);
    Move(aName[1], Result[1], aLength); // faster than system.move
  end;
end;
...
Var RawXmlString: AnsiString;
  ...
  Begin
    If RawXmlStringPos > 1 then begin
      ...
      if RawXmlStringPos <= length(RawXmlString) then Move(RawXmlString[RawXmlStringPos],
                                                             RawXmlString[1],
...
Var NodeStack: Tstack;
  ...
  Begin
    ...
    if L >= BufferSize then WriteBuffer2Stream(str, l)
      ...
      Move(str[1], BufferString[BufferStringPos + 1], L);
...
Var aStringStream: TStringStream;
Begin
  aStringStream := TStringStream.Create(Value);
  Try
...
begin
  ...
  if Index = -1 then begin
    ...
    if Index < FCount then Move(FList^[Index],
                                  FList^[Index + 1],
...
begin
  Node := Get(Index);
  Dec(FCount);
  if Index < FCount then Move(FList^[Index + 1],
                                FList^[Index],
                                (FCount - Index) * SizeOf(Pointer));
...
Begin
  ...
  for i := 0 to xmlrec.ChildNodes.Count - 1 do begin
    for J := 0 to xmlrec.ChildNodes[i].ChildNodes.Count - 1 do begin
      If Sametext(xmlrec.ChildNodes[i].ChildNodes[j].NodeName,ChildNodeName) and
         Sametext(xmlrec.ChildNodes[i].ChildNodes[j].text,ChildNodeValue) then begin
...
Begin
  ...
  if not (xmlrec is TalXmlElementNode) then Exit;
  for i := 0 to xmlrec.ChildNodes.Count - 1 do begin
    if Sametext(xmlrec.ChildNodes[i].NodeName,NodeName) then begin
      for J := 0 to xmlrec.ChildNodes[i].ChildNodes.Count - 1 do begin
        If Sametext(xmlrec.ChildNodes[i].ChildNodes[j].NodeName,ChildNodeName) and
           Sametext(xmlrec.ChildNodes[i].ChildNodes[j].text,ChildNodeValue) then begin
          result := xmlrec.ChildNodes[i];
...
Begin
  ...
  if not (xmlrec is TalXmlElementNode) then Exit;
  for i := 0 to xmlrec.ChildNodes.Count - 1 do begin
    If Sametext(xmlrec.ChildNodes[i].Attributes[AttributeName], AttributeValue) then begin
      result := xmlrec.ChildNodes[i];
      break;
    end;
    if Recurse then begin
...
Begin
  ...
  if not (xmlrec is TalXmlElementNode) then Exit;
  for i := 0 to xmlrec.ChildNodes.Count - 1 do begin
    If Sametext(xmlrec.ChildNodes[i].NodeName, NodeName) and
       Sametext(xmlrec.ChildNodes[i].Attributes[AttributeName], AttributeValue) then begin
      result := xmlrec.ChildNodes[i];
      break;
    end;
    if Recurse then begin
