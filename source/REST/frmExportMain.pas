...
adaption for mX4  with own synedit
...
uses
  uHighlighterProcs, Clipbrd, fmain;

{ TForm1 }

procedure TSynexportform.FormCreate(Sender: TObject);
begin
  SynPasSyn1:= maxform1.SynPasSyn1;
  ...
  //SynPasSyn1:= maxform1.SynPasSyn1;
  //fHighlighters.
  if fileexists(ExtractFilePath(Forms.Application.ExeName)+'firstdemo.txt') then
    SynEdit1.Lines.LoadFromFile(ExtractFilePath(Forms.Application.ExeName)+'firstdemo.txt');
  synedit1.Highlighter:= maxform1.SynPasSyn1;

 end;

procedure TSynexportform.FormDestroy(Sender: TObject);
begin
  fHighlighters.Free;
end;

procedure TSynexportform.miFileOpenClick(Sender: TObject);
begin
  if dlgFileOpen.Execute then begin
    ...
    if ExtractFileExt(dlgFileOpen.FileName) = '.txt' then
       synedit1.Highlighter:= maxform1.SynPasSyn1;
    if ExtractFileExt(dlgFileOpen.FileName) = '.pas' then
       synedit1.Highlighter:= maxform1.SynPasSyn1;
    //ChangeFileExt(
...
procedure TSynexportform.miFileExitClick(Sender: TObject);
begin
  Close;
end;

procedure TSynexportform.mExportClick(Sender: TObject);
var
...
procedure TSynexportform.miExportToFileClick(Sender: TObject);
var
  FileName: string;
...
procedure TSynexportform.miExportAsClicked(Sender: TObject);
begin
...
procedure TSynexportform.miExportClipboardNativeClick(Sender: TObject);
begin
  Clipboard.Open;
  try
...
procedure TSynexportform.miExportClipboardTextClick(Sender: TObject);
var
  Exporter: TSynCustomExporter;
begin
  Exporter := nil;
