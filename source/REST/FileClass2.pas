...
type
  ...
  TIFileAccess = (faRead, faWrite, faReadWrite);
  TIFileSharing = (fsNone, fsRead, fsWrite, fsReadWrite);
  ...
  public
    ...
    function Read(var Buffer: string; Count: Cardinal): Cardinal; virtual; abstract;
    procedure ReadBuffer(var Buffer: string; Count: Cardinal);
    procedure Seek(Offset: Cardinal);
    procedure Seek64(Offset: Integer64); virtual; abstract;
    procedure WriteAnsiString(const S: AnsiString);
    procedure WriteBuffer(const Buffer: string; Count: Cardinal); virtual; abstract;
  ...
  TIFile = class(TCustomFile)
  private
    FHandle: THandle;
    FHandleCreated: Boolean;
  protected
    function CreateHandle(const AFilename: String;
      ACreateDisposition: TFileCreateDisposition; AAccess: TIFileAccess;
      ASharing: TIFileSharing): THandle; virtual;
    function GetPosition: Integer64; override;
    function GetSize: Integer64; override;
  public
    constructor Create(const AFilename: String;
      ACreateDisposition: TFileCreateDisposition; AAccess: TIFileAccess;
      ASharing: TIFileSharing);
    constructor CreateWithExistingHandle(const AHandle: THandle);
    destructor Destroy; override;
    function Read(var Buffer: string; Count: Cardinal): Cardinal; override;
    procedure Seek64(Offset: Integer64); override;
    procedure SeekToEnd;
    procedure Truncate;
    procedure WriteBuffer(const Buffer: string; Count: Cardinal); override;
    property Handle: THandle read FHandle;
  end;

  TMemoryFile = class(TCustomFile)
  private
    FMemory: string; //Pointer;
    FSize: Integer64;
    FPosition: Integer64;
  ...
  TTextFileReader = class(TIFile)
  private
    FBufferOffset, FBufferSize: Cardinal;
    FEof: Boolean;
    FBuffer: string; //array[0..4095] of AnsiChar;
{$IFDEF UNICODE}
...
{$ENDIF}
  ...
  protected
    function CreateHandle(const AFilename: String;
      ACreateDisposition: TFileCreateDisposition; AAccess: TIFileAccess;
      ASharing: TIFileSharing): THandle; override;
  public
...
{$ENDIF}
  ...
  public
    constructor Create(AFile: TIFile; AWritable: Boolean);
    destructor Destroy; override;
    procedure Commit;
...
procedure TCustomFile.ReadBuffer(var Buffer: string; Count: Cardinal);
begin
...
constructor TIFile.Create(const AFilename: String;
  ACreateDisposition: TFileCreateDisposition; AAccess: TIFileAccess;
  ASharing: TIFileSharing);
begin
  inherited Create;
...
constructor TIFile.CreateWithExistingHandle(const AHandle: THandle);
begin
  inherited Create;
  FHandle := AHandle;
end;
...
function TIFile.CreateHandle(const AFilename: String;
  ACreateDisposition: TFileCreateDisposition; AAccess: TIFileAccess;
  ASharing: TIFileSharing): THandle;
const
  AccessFlags: array[TIFileAccess] of DWORD =
    (GENERIC_READ, GENERIC_WRITE, GENERIC_READ or GENERIC_WRITE);
  SharingFlags: array[TIFileSharing] of DWORD =
...
function TIFile.GetPosition: Integer64;
begin
  Result.Hi := 0;
...
function TIFile.GetSize: Integer64;
begin
...
function TIFile.Read(var Buffer: string; Count: Cardinal): Cardinal;
begin
...
procedure TIFile.Seek64(Offset: Integer64);
begin
...
procedure TIFile.Truncate;
begin
  if not SetEndOfFile(FHandle) then
    RaiseLastError;
end;

procedure TIFile.WriteBuffer(const Buffer: string; Count: Cardinal);
var
  BytesWritten: DWORD;
begin
...
begin
  inherited Create;
  F := TIFile.Create(AFilename, fdOpenExisting, faRead, fsRead);
  try
...
function TMemoryFile.Read(var Buffer: string; Count: Cardinal): Cardinal;
begin
...
procedure TMemoryFile.WriteBuffer(const Buffer: string; Count: Cardinal);
begin
...
constructor TFileMapping.Create(AFile: TIFile; AWritable: Boolean);
const
...
{ In Delphi, when an I/O error occurs while accessing a memory-mapped file --
  ...
  you'd see when doing non-memory-mapped I/O with TIFile. }
var
  E: TObject;
begin
  E := ExceptObject;
  if (E is EExternalException) and
    ...
    if (Cardinal(EExternalException(E).ExceptionRecord.NumberParameters) >= Cardinal(3)) and
      ...
      TIFile.RaiseError(_RtlNtStatusToDosError(EExternalException(E).ExceptionRecord.ExceptionInformation[2]))
    else begin
      ...
      if EExternalException(E).ExceptionRecord.ExceptionInformation[0] = 0 then
        TIFile.RaiseError(ERROR_READ_FAULT)
      else
        TIFile.RaiseError(ERROR_WRITE_FAULT);
    end;
  end;
end;
