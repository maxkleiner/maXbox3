...
const
  UnitVersioning: TUnitVersionInfo = (
    RCSfile: '$URL: https://jcl.svn.sourceforge.net/svnroot/jcl/tags/JCL-1.102-Build3072/jcl/source/windows/JclMiscel.pas $';
...
begin
  //Result := Execute(Cmd, Output, RawOutput);      to do 
end;
...
begin
  {$IFDEF MSWINDOWS}
  Result := JclMiscel2.ExitWindows(EWX_LOGOFF or KillLevelToFlags(KillLevel));
  {$ENDIF MSWINDOWS}
...
begin
  {$IFDEF MSWINDOWS}
  Result := JclMiscel2.ExitWindows(EWX_POWEROFF or KillLevelToFlags(KillLevel));
  {$ENDIF MSWINDOWS}
end;
...
begin
  {$IFDEF MSWINDOWS}
  Result := JclMiscel2.ExitWindows(EWX_SHUTDOWN or KillLevelToFlags(KillLevel));
  {$ENDIF MSWINDOWS}
end;
...
begin
  {$IFDEF MSWINDOWS}
  Result := JclMiscel2.ExitWindows(EWX_REBOOT or KillLevelToFlags(KillLevel));
  {$ENDIF MSWINDOWS}
end;
...
begin
  ...
  begin
    case GetLastError of
      ...
      ERROR_LOGON_FAILURE:
        raise EJclCreateProcessError.Create(RsCreateProcLogonUserError);
      ERROR_ACCESS_DENIED:
        raise EJclCreateProcessError.Create(RsCreateProcAccessDenied);
    else
      //raise EJclCreateProcessError.CreateRes(@RsCreateProcLogonFailed);
      raise EJclCreateProcessError.Create(RsCreateProcLogonFailed);
  ...
  begin
    case GetLastError of
      ...
      ERROR_FILE_NOT_FOUND:
        ...
        raise EJclCreateProcessError.Create(RsCreateProcFailed);
    end;
  end;
