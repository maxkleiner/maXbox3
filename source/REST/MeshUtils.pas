...
procedure ConvertStripToList(const strip : TXIntegerList;
                             list : TXIntegerList); overload;
procedure ConvertStripToList(const strip : TAffineVectorList;
                             const indices : TXIntegerList;
                             list : TAffineVectorList); overload;
...
function BuildVectorCountOptimizedIndices(const vertices : TAffineVectorList;
                                          ...
                                          const texCoords : TAffineVectorList = nil) : TXIntegerList;
...
procedure RemapReferences(reference : TAffineVectorList;
                          const indices : TXIntegerList); overload;
procedure RemapReferences(reference : TXIntegerList;
                          const indices : TXIntegerList); overload;
...
function RemapIndicesToIndicesMap(remapIndices : TXIntegerList) : TXIntegerList;
...
procedure RemapTrianglesIndices(indices, indicesMap : TXIntegerList);
{: Remaps a list of indices.<p>
   The indicesMap provides newVertexIndex:=indicesMap[oldVertexIndex] }
procedure RemapIndices(indices, indicesMap : TXIntegerList);
...
{: Attempts to unify triangle winding.<p>
   ...
   submeshes etc.) }
procedure UnifyTrianglesWinding(indices : TXIntegerList);
{: Inverts the triangles winding (vertex order). }
procedure InvertTrianglesWinding(indices : TXIntegerList);
...
function BuildNormals(reference : TAffineVectorList;
                      indices : TXIntegerList) : TAffineVectorList;
...
{: Builds a list of non-oriented (non duplicated) edges list.<p>
   ...
   orientation. }
function BuildNonOrientedEdgesList(triangleIndices : TXIntegerList;
                                   triangleEdges : TXIntegerList = nil;
                                   edgesTriangles : TXIntegerList = nil) : TXIntegerList;
...
{: Attempts to create as few as possible triangle strips to cover the mesh.<p>
   ...
   The function returns a list of TXIntegerList, each of these lists hosting
...
function StripifyMesh(indices : TXIntegerList; maxVertexIndex : Integer;
...
procedure IncreaseCoherency(indices : TXIntegerList; cacheSize : Integer);
...
var
   ...
   found : Boolean;
   hashSize : Integer;
   hashTable : array of TXIntegerList;
   list : TXIntegerList;
...
begin
   ...
   for i:=0 to hashSize do begin
      hashTable[i]:=TXIntegerList.Create;
...
var
   ...
   triangleStack : TXIntegerList; // marks triangles winded, that must be processed
...
begin
   ...
   triangleStack:=TXIntegerList.Create;
   try
...
var
   edgesHash : array [0..cEdgesHashMax] of TXIntegerList;
   curTri : Integer;
   edges : TXIntegerList;
...
begin
   ...
   for j:=0 to High(edgesHash) do begin
      edgesHash[j]:=TXIntegerList.Create;
...
var
   ...
   trisOfVertex : array of TXIntegerList;
   candidates : TXIntegerList;
   indicesList : PIntegerArray;
begin
   ...
   for i:=0 to High(trisOfVertex) do
      trisOfVertex[i]:=TXIntegerList.Create;
   candidates:=TXIntegerList.Create;
...
var
   ...
   vertexTris : array of TXIntegerList;
   indicesList : PIntegerArray;
   indicesCount : Integer;
   currentStrip : TXIntegerList;
...
begin
   ...
   for i:=0 to High(vertexTris) do
      vertexTris[i]:=TXIntegerList.Create;
   n:=0;
   triangle:=0;
   ...
   if agglomerateLoneTriangles then begin
      loneTriangles:=TXIntegerList.Create;
   ...
   i:=0; while i<indicesCount do begin
      if not accountedTriangles[i] then begin
         ...
         if FindTriangleWithEdge(indicesList[i+1], indicesList[i]) then begin
            currentStrip:=TXIntegerList.Create;
            ...
            else currentStrip:=TXIntegerList.Create;
...
var
   i, a, b, c, nv : Integer;
   edges : TXIntegerList;
   triangleEdges : TXIntegerList;
   p, n : TAffineVector;
   f : Single;
begin
   // build edges list
   triangleEdges:=TXIntegerList.Create;
   try
