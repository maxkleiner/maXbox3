   ...
   TBaseVectorList = class (TBaseList)
	...
	TXIntegerList = class (TBaseList)
		...
		public
         ...
         procedure Add(const list : TXIntegerList); overload;
         ...
         procedure AddIntegers(const aList : TXIntegerList); overload;
...
// Assign
//
procedure TXIntegerList.Assign(Src: TPersistent);
begin
	if Assigned(Src) then begin
      inherited;
		if (Src is TXIntegerList) then
			System.Move(TXIntegerList(Src).FList^, FList^, FCount*SizeOf(Integer));
	end else Clear;
end;

// Add (simple)
//
function TXIntegerList.Add(const item : Integer) : Integer;
begin
	Result:=FCount;
	if Result=FCapacity then SetCapacity(FCapacity+FGrowthDelta);
	FList[Result]:=Item;
...
//
function TXIntegerList.AddNC(const item : Integer) : Integer;
begin
	Result:=FCount;
	FList[Result]:=Item;
  	Inc(FCount);
...
// Add (two at once)
//
procedure TXIntegerList.Add(const i1, i2 : Integer);
var
   list : PIntegerArray;
begin
  	Inc(FCount, 2);
...
// Add (three at once)
//
procedure TXIntegerList.Add(const i1, i2, i3 : Integer);
var
   list : PIntegerArray;
begin
  	Inc(FCount, 3);
...
// Add (list)
//
procedure TXIntegerList.Add(const list : TXIntegerList);
begin
   if Assigned(list) and (list.Count>0) then begin
      if Count+list.Count>Capacity then
         Capacity:=Count+list.Count;
...
// Get
//
function TXIntegerList.Get(Index: Integer): Integer;
begin
{$IFOPT R+}
   Assert(Cardinal(Index)<Cardinal(FCount));
{$endif}
...
// Insert
//
procedure TXIntegerList.Insert(Index: Integer; const Item: Integer);
begin
{$IFOPT R+}
   Assert(Cardinal(Index)<Cardinal(FCount));
{$ENDIF}
...
// Remove
//
procedure TXIntegerList.Remove(const item : Integer);
var
   i : Integer;
begin
   for i:=0 to Count-1 do begin
...
// Put
//
procedure TXIntegerList.Put(Index: Integer; const Item: Integer);
begin
{$IFOPT R+}
   Assert(Cardinal(Index)<Cardinal(FCount));
{$ENDIF}
...
// SetCapacity
//
procedure TXIntegerList.SetCapacity(NewCapacity: Integer);
begin
   inherited;
...
// Push
//
procedure TXIntegerList.Push(const val : Integer);
begin
	Add(val);
end;
...
// Pop
//
function TXIntegerList.Pop : Integer;
begin
	if FCount>0 then begin
...
// AddSerie
//
procedure TXIntegerList.AddSerie(aBase, aDelta, aCount : Integer);
var
   list : PInteger;
   i : Integer;
begin
...
//
procedure TXIntegerList.AddIntegers(const first : PInteger; n : Integer);
begin
   if n<1 then Exit;
...
//
procedure TXIntegerList.AddIntegers(const aList : TXIntegerList);
begin
...
// AddIntegers (array)
//
procedure TXIntegerList.AddIntegers(const anArray : array of Integer);
var
   n : Integer;
begin
   n:=Length(anArray);
...
// IndexOf
//
function TXIntegerList.IndexOf(item : Integer) : Integer; register;
begin
...
// MinInteger
//
function TXIntegerList.MinInteger : Integer;
var
   i : Integer;
...
// MaxInteger
//
function TXIntegerList.MaxInteger : Integer;
var
   i : Integer;
   locList : PIntegerVector;
begin
...
//
procedure TXIntegerList.SortAndRemoveDuplicates;
var
	i, j, lastVal : Integer;
...
// BinarySearch
//
function TXIntegerList.BinarySearch(const value : Integer) : Integer;
var
   found : Boolean;
begin
...
// BinarySearch
//
function TXIntegerList.BinarySearch(const value : Integer; returnBestFit : Boolean;
...
// AddSorted
//
function TXIntegerList.AddSorted(const value : Integer; const ignoreDuplicates : Boolean = False) : Integer;
var
   index : Integer;
   found : Boolean;
begin
...
// RemoveSorted
//
procedure TXIntegerList.RemoveSorted(const value : Integer);
var
   index : Integer;
begin
...
// Offset (all)
//
procedure TXIntegerList.Offset(delta : Integer);
var
   i : Integer;
...
// Offset (range)
//
procedure TXIntegerList.Offset(delta : Integer; const base, nb : Integer);
var
   i : Integer;
   locList : PIntegerArray;
begin
