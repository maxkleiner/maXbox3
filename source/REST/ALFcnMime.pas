          ...
          if (length(aExt) > 1) and (aExt[1] = '.') then begin
            if reg.OpenKeyReadOnly('\' + aExt) then begin
              ...
              If (length(aExt) > 1) then begin
                ...
                if aContentType <> '' then AMIMEList.Values[LowerCase(ansiString(aExt))] := LowerCase(ansistring(aContentType));
              end;
              Reg.CloseKey;
            end;
          end;
            ...
            if Reg.OpenKeyreadOnly('\MIME\Database\Content Type\' + aContentType) then begin
              ...
              if aContentType <> '' then begin
                ...
                if aExt <> '' then begin
                  ...
                  AMIMEList.Values[LowerCase(ansiString(aContentType))] := LowerCase(ansistring(aExt))
                end;
              end;
              Reg.CloseKey;
            end;
