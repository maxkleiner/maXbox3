...
add with MOD as % 4.4.2012 and Fact, !  , a&b is like a^x=b = logN(a,b), GCD=], LCM=[

Known Issues: for maXbox as maXcalc()
-----------------------------------------------------------------------------}
...
type
  TParserFunc = (pfArcTan, pfCos, pfSin, pfTan, pfAbs, pfExp, pfLn, pfLog,
    ...
    pfSign, pfNot, pfMod, pfFact, pfLogN, pfGCD, pfLCM, pfDIV, pfRND);
...
const
  ...
  FuncNames: array [TParserFunc] of PChar =
    ('ARCTAN', 'COS', 'SIN', 'TAN', 'ABS', 'EXP', 'LN', 'LOG',
     ...
     'SIGN', 'NOT','MOD','FACT','LOGN','GCD','LCM','DIV','RND');
        ...
        if GetFunction(NoFunc) then begin
          ...
          try
            case NoFunc of
              ...
              pfFact:
                value:= Fact(round(Value));
              pfRND: value:= random;  
              //pfLogN:
                //value:= LogN(Value,Term);
                //value:= LN(Value)/LN(Value);
              //pfGCD:
                //value:= GCD(round(Value), round(Term));
...
begin
  ...
  while FParseText[FCurPos] in ['*', '^', '/', '!','%','[',']','&','@'] do begin
    ...
    if FParseText[FCurPos-1] = '%' then
      Value:= (round(Value) mod round(Term))
    else
     if FParseText[FCurPos-1] = ']' then
      Value:= GCD(round(Value), round(Term))
    else
     if FParseText[FCurPos-1] = '[' then
      Value:= LCM(round(Value), round(Term))
    else
