...
begin
  ...
  AddType('OLEVariant', btVariant);   //3.6  test
...
begin
  ...
  //by max
   AddFunction('function IntToHex(a: integer; b: integer): string;');
   AddFunction('procedure Assert(expr : Boolean; const msg: string);');
  //AddFunction('function Power(const Base, Exponent: Extended): Extended);');
  AddFunction('function Frac(X: Extended): Extended);');
  //AddFunction('function LogN(const Base, X: Extended): Extended;');
  AddFunction('function Ln(x: Extended): Extended;');
  AddFunction('function Exp(x: Extended): Extended;');
  //AddFunction('function DateTimeToStr(datetime: extended): string;');
  AddFunction('function Odd(x: Longint): boolean;');
  ...
  p := AddFunction('function ArrayLength: integer;');  //3.8 mX
  with P.Decl.AddParam do
  begin
    OrgName := 'arr';
    Mode := pmInOut;
  ...
  AddFunction('Function Sqr(e : Extended) : Extended;'); //max
...
begin
//add 2.8.1 and 3.8 to mX4
  if ErrorType = 'Hint' then
  Result:= '['+ErrorType+'] '+extractFileName(FModuleName)+
       '('+IntToStr(FRow)+':'+IntToStr(FCol)+'): '+ShortMessageToString else
