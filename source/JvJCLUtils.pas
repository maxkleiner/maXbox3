...
begin
  ...
  LenStr := Length(S);
  if (LenSubStr = 0) or (S = nativeSpace) or (LenSubStr > LenStr - (Index - 1)) then
    Exit;
...
begin
  //TempDir:=
  getTempPath(Length(TempDir)+1,@TempDir[1]);
  //initTempPath; //Length(;TempPath)+1,@TempPath[1])gettemppath//PathGetTempPath;
  if FileName <> '' then
  begin
...
function CharIsMoney(const Ch: Char): Boolean;
begin
  Result := CharIsDigit(Ch) or (Ch = '') or (Ch = '$') or (Ch = '-') or
...
begin
  ...
  begin
    ...
    if Ch = '/' then //NativeForwardSlash then
      Inc(liSlashes)
    else
