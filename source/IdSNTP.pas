...
{**********************************************************************}
{}
{ $Log:  10331: IdSNTP.pas one change for maXbox
{
...
begin
  ...
  if Value2 > NTPMaxInt then begin
...
begin
  ...
  if FCheckStratum and ((vStratum > 15) or (vStratum = 0)) then begin
    Result := True;
  end;
end;
...
begin
  ...
  if Length(ResultString) >= SizeOf(NTPDataGram) then begin
