...
and updated by max.  works just with a DB Source
...
begin
  ...
  with CL.AddClassN(CL.FindClass('TDBLookupListBox'),'TPopupDataList') do
  begin
  end;
end;
...
begin
  ...
  with CL.AddClassN(CL.FindClass('TSpeedButton'),'TNavButton') do
  begin
...
begin
 ...
 CL.AddConstantN('SpaceSize','LongInt').SetInt( 5);
  ...
  CL.AddTypeS('TNavigateBtn', '( nbFirst, nbPrior, nbNext, nbLast, nbInsert, nb'
   +'Delete, nbEdit, nbPost, nbCancel, nbRefresh )');
  CL.AddTypeS('TNavButtonSet', 'set of TNavigateBtn');
  //CL.AddTypeS('TNavButtonStyle', 'set of ( nsAllowTimer, nsFocusRect )');
  CL.AddTypeS('TNavButtonStyle', '( nsAllowTimer, nsFocusRect )');
...
procedure RIRegister_TListSourceLink(CL: TPSRuntimeClassImporter);
begin
  with CL.Add(TListSourceLink) do
  begin
