...
begin
  ...
  begin
    ...
    try
      if BitsPerPixel>8 then AssignTo(DelphiBitmap,Transparent)
...
begin
  ...
  begin
    ...
    if (BitmapHeader.biWidth<>DirEntry.bWidth) or (BitmapHeader.biHeight div 2<>DirEntry.bHeight) then
      raise Exception.CreateFmt('Size mismatch in icon %d',[I-1]);
    ...
    begin
      ...
      begin
//        Stream.Read(Palette^,PaletteSize);
...
//    ZeroMem(fIcons^,Dir.idCount*SizeOf(TIconImage));
