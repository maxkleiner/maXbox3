...
begin
  inherited Create(NextStream);
	...
	LittLengthTree:=THuffmanTree.Create;//mano
	DistTree :=THuffmanTree.Create;
	LittLengthCodes :=THuffmanCodes.Create;
	DistCodes:= THuffmanCodes.Create;


  if Next.CanWrite and ((Mode=-1) or (Mode=fmWrite)) then begin
    fCanWrite:=True;
    GetMem(HashTable,SizeOf(TDeflateHashTable));
    FillChar(HashTable^,SizeOf(TDeflateHashTable),$80);
    New(HuffmanBuffer);
  end
  else if Next.CanRead and ((Mode=-1) or (Mode=fmRead)) then begin
    fCanRead:=True;
...
begin
  ...
  FreeMem(LookBack);
	...
	LittLengthTree.Free;//mano
	DistTree.Free;
	
	LittLengthCodes.Free;//mano
	DistCodes.Free;//mano
  //CodeLengthCodes.Free;  in huffmancompressdata
end;
...
begin
CodeLengthTree:= THuffmanTree.Create;
try
...
type
  ...
  begin
    if not InBlock then begin // Start new block
    ...
    while (BufferSize>=258) or (FinishBlock and (BufferSize>=3)) do begin
      // Find hash value
      ...
      BestLength:=2;
      with HashTable^[HashValue.Index] do
        for I:=MaxDeflateHashChain-1 downto 0 do begin// Try nearest match first for shorter distance codes
          Dist:=Position-Chain[(I+Next) and (MaxDeflateHashChain-1)]+1;
          if (Dist>0) and (Dist<=32768) then begin
      ...
      begin
        // Write length code
        if BestLength<=10 then begin
          with LittLengthCodes.List[254+BestLength] do WriteBits(Code,Length);
        end
        else if BestLength<=257 then begin// 1-5 extra bits
  ...
  procedure CompressDataAuto; // Compress data using optimal Huffman codes
    ...
    begin
      // Write block header
      if (BufferSize=0) and Final then begin
        WriteBits(Cardinal(Final),1); // Write Final bit
        Final:=False;
      end
      ...
      with HuffmanBuffer^[HuffmanBufferPos] do begin
        Value:=256;
        Info:=LitLengthValue;
      end;
      ...
      for I:=285 downto 1 do if LittLengthCodes.List[I].Length<>0 then begin
        LittLengthCodeCount:=I+1;
        Break;
      end;
      DistCodeCount:=1;
      for I:=29 downto 1 do if DistCodes.List[I].Length<>0 then begin
        DistCodeCount:=I+1;
        Break;
      end;
      ...
      for I:=1 to LittLengthCodeCount+DistCodeCount do if CodeLength[I]=LastCode then Inc(CodeCount)
        ...
        while CodeCount>=3 do begin
          if LastCode<>0 then begin// Repeat 3-6 times
            CompCodeLength[CompCodeCount].Info:=-1;
          ...
          else if CodeCount<=10 then begin// 3-10 x 0
            CompCodeLength[CompCodeCount].Info:=-1;
      ...
      for I:=0 to CompCodeCount-1 do begin
  ...
  begin
    ...
    begin
      ...
      if BestLength>=3 then begin// At least 3 characters found in look-back
        // Write length code
        if BestLength<=10 then begin
          with HuffmanBuffer^[HuffmanBufferPos] do begin
...
begin
  ...
  Result:=0;
  while (Count>0) or FinishBlock do begin
...
begin
  while CanWrite and ((BufferSize>0) or InBlock or Final) do begin
    FinishBlock:=True;
