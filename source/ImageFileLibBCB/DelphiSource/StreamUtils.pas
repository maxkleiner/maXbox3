...
uses Monitor, SysUtils, Classes, Streams, BufStream, MemUtils;

procedure SaveString(Str: string; Stream: TBaseStream);
procedure SaveString16(Str: string; Stream: TBaseStream);
...
type
  TStreamClass = class(TAssignObject)
    public
      ...
      procedure LoadFromStream(Stream: TBaseStream); virtual;
...
begin
  raise Exception.Create(ClassName+'.SaveToStream not implemented');
end;

procedure TStreamClass.LoadFromStream(Stream: TBaseStream);
begin
  raise Exception.Create(ClassName+'.LoadFromStream not implemented');
end;
