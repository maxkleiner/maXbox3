...
uses Classes, SysUtils, LinarBitmap, Streams, BufStream, Graphics, DelphiStream, Monitor;
...
type
  TPCXLoader = class(TBitmapLoader)
              public
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if Plan=0 then
          begin
            if Assigned(ProgressUpdate) then ProgressUpdate(X*100 div Size);
            Plan:=2
          end
          else
          begin
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          if ((X>0) and (X mod Width=0)) or (Farv<>Map^[X]) or (Ant=63) then
          begin
        ...
        if Ant<>0 then  // Write last block/pixel
        begin
          if (Ant<>1) or (Farv and $c0=$c0) then
          begin
            Code:=Ant or $c0;
