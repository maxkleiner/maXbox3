...
uses
  ...
  SysUtils, Classes, Grids, Controls, Forms, MMSystem,
  JvgTypes, //Jvg3DColors,
   JvgCommClasses; //, Jvg3DColors;
...
uses
  JvVCLUtils,
  ShlObj, Math, IFSI_WinForm1puzzle,
  //JvResources,
   JvConsts;
...
begin
  if AColor < 0 then
    AColor := GetSysColor(AColor and $FF);
  R := math.Max(0, GetRValue(AColor) - AOffset);
  G := math.Max(0, GetGValue(AColor) - AOffset);
  B := math.Max(0, GetBValue(AColor) - AOffset);
  Result := RGB(R, G, B);
end;
...
begin
  try
    if Assigned(glGlobalData.lp3DColors) then
      {with TJvg3DColors(glGlobalData.lp3DColors) do begin
...
begin
  ...
  begin
    ...
    begin
      ...
      Shadow := RGB(math.Max(R - ColorShadowShift, 0), math.Max(G - ColorShadowShift, 0),
             math.Max(B - ColorShadowShift, 0));
    end;
...
begin
  with Grid do begin
    ...
    Result:= not(Grid.Selection.Top = RowCount-1)and(Grid.Selection.Left = ColCount-1);
    if Result then
    if Selection.Top = RowCount-1 then begin
...
begin
  ...
  repeat
    ...
    if TextHeight < Size.cy then
      TextHeight := Size.cy;
    if (LineWidth > Width) or (Pos2 >= Length(Text)) then begin
...
function ComputerName: string;
begin
  Result := IFSI_WinForm1puzzle.GetComputerNameWin;
end;
