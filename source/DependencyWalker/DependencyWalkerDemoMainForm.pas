{******************************************************************
 ...
 add filters and formats by max
...
type
  ...
  TDependencyWalkerDemoMainFrm = class(TfrmPersistable)
    ...
    acNoSort: TAction;
    none1: TMenuItem;
    tbnsaveshape: TToolButton;
    ToolButton5: TToolButton;
    ToolButton6: TToolButton;
    ToolButton8: TToolButton;
    btnopenshape: TToolButton;
...
begin
  Changed := false;
  for i := 0 to AShape.Parent.ControlCount - 1 do begin
...
begin
  ...
  LoadSettings;
  if FileExists(ExtractFilePath(Application.Exename)+'\examples\units_explorer_ref.pac') then begin
  TJvCustomDiagramShape.LoadFromFile(ExtractFilePath(Application.Exename)+'\examples\units_explorer_ref.pac', sb);
  statusBar1.SimpleText:= 'Pattern File of \examples\units_explorer_ref.pac has been loaded';
  end else  Showmessage('File not ready: \examples\units_explorer_ref.pac')
  //default for first show a diagram
end;
...
begin
  ...
  statusBar1.SimpleText:= 'File '+dlgSaveImage.Filename+' been saved';
end;
...
begin
  with TSaveDialog.Create(nil) do begin
    ...
    Title:= 'Save Diagram Image as File PNG';
    DefaultExt:= 'pac';
  try
    if Execute then
      TJvCustomDiagramShape.SaveToFile(Filename, sb);
   statusBar1.SimpleText:= 'File '+Filename+' been saved';
  finally
    Free;
  end;
  end;
...
begin
  with TOpenDialog.Create(nil) do begin
    Filter:= 'PAC files (*.pac)|*.pac|All files|*.*';
    Options:= [ofOverwritePrompt, ofHideReadOnly, ofEnableSizing];
    Title:= 'Open Diagram Image from File';
    DefaultExt:= 'pac';
  try
    if Execute then
    begin
     ...
     statusBar1.SimpleText:= 'File '+Filename+' has been opened';
...
begin
  //Result:= TPersistStorage(TMemIniFile.Create(ChangeFileExt(Application.ExeName+'explorer', cIniFileExt)));
  Result:= TPersistStorage(TMemIniFile.Create('maxboxunit'+cIniFileExt));
