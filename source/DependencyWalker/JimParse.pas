...
type
  ...
  TjimSymbolType = (stTitle,stBase,stLink,stImage,stdoc,stzip,stmp3);
...
begin {AttributeList}
  while FLookahead.AsString <> '>' do begin
    ...
    if FLookahead.AsString = '=' then begin
      ...
      end else if (CompareText(FLastTag,'PDF') = 0) and
         (CompareText('HREF',FLastAttribute) = 0) then begin
        // Found an image
        FSymbolTable.AddSymbol(stDoc,FLastValue);
      end else if (CompareText(FLastTag,'ZIP') = 0) and
         (CompareText('SRC',FLastAttribute) = 0) then begin
        // Found an image
        FSymbolTable.AddSymbol(stzip,FLastValue);
       end else if (CompareText(FLastTag,'MP3') = 0) and
         (CompareText('SRC',FLastAttribute) = 0) then begin
        // Found an image
        FSymbolTable.AddSymbol(stmp3,FLastValue);
