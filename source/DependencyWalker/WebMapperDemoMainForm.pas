...
type
  TWebMapperDemoMainFrm = class(TForm)
    ...
    //TJvUrlListGrabber
    procedure JvHttpGrabber1DoneStream(Sender: TObject;
        ...
        case SymbolTable.Items[i].SymbolType of
          ...
          stLink:
            begin
              ...
              if StrLIComp('ftp://', PChar(TempStr), 6) = 0 then begin
                ...
                TempIndex := 0;
              end else if StrLIComp('pdf', PChar(TempStr), 3) = 0 then begin
                TempIndex := 5;
          ...
          stDoc:
            begin
              ...
              // Create diagram component for this URL, and link to diagram
              // component for current URL
              ChildShape:= CreateBitmapShape(TempStr, ChildX, FNextChildY, 5);
              Inc(FNextChildY, ChildDY);
              // Connect this shape to the current document component
              ConnectShapes(CurrShape, ChildShape);
            end;
          stZip:
            begin
              ...
              // Create diagram component for this URL, and link to diagram
              // component for current URL
              ChildShape:= CreateBitmapShape(TempStr, ChildX, FNextChildY, 6);
              Inc(FNextChildY, ChildDY);
              // Connect this shape to the current document component
              ConnectShapes(CurrShape, ChildShape);
            end;
          stMP3:
            begin
              ...
              // Create diagram component for this URL, and link to diagram
              // component for current URL
              ChildShape:= CreateBitmapShape(TempStr, ChildX, FNextChildY, 7);
              Inc(FNextChildY, ChildDY);
              // Connect this shape to the current document component
              ConnectShapes(CurrShape, ChildShape);
            end;
...
procedure TWebMapperDemoMainFrm.Runfirst;
begin
  parseBtnclick(self);
end;
...
var defprop: TJvCustomUrlGrabberDefaultProperties;
    myprop: TJvUrlGrabberDefaultPropertiesList;
    mygrab: TJvUrlListGrabber;
begin
  FCurrentUrl:= 'www.softwareschule.ch/maxbox.htm';
  FParentUrlList:= TStringList.Create;
  mygrab:= TJvUrlListGrabber.create(self);
  myprop:= TJvUrlGrabberDefaultPropertiesList.Create(mygrab);
  //myprop.
  defprop:= TJvCustomUrlGrabberDefaultProperties.Create(myprop);
  //JvHttpGrabber1:= TJvHttpUrlGrabber.Create(self, FCurrentUrl, defprop);
  JvHttpGrabber1:= TJvHttpUrlGrabber.Create(self, FCurrentUrl, defprop);
    with JvHttpGrabber1 do begin
    FileName:= 'grabberoutput.txt';
    OutputMode:= omStream;
    Agent:= 'TJvHttpGrabber Component';
    //agent:= 'JvHttpGrabber1';
    Port:= 80;
...
procedure TWebMapperDemoMainFrm.btnclearClick(Sender: TObject);
var i: integer;
begin
  ClearAll(FCurrentShape);
  //ClearAll(NIL);
  for I:= Scrollbox1.ComponentCount - 1 downto 0 do
    TJvCustomDiagramShape(Scrollbox1.Components[I]).free;
  scrollbox1.Invalidate;
  //scrollbox1.
end;

procedure TWebMapperDemoMainFrm.Button1Click(Sender: TObject);
begin
  with opendialog1 do begin
    if execute then
       urledit.text:= filename;
  end;
end;
...
//    Grabber: TJvCustomUrlGrabber; Stream: TStream; StreamSize: Integer; Url: string);
   Stream: TStream; StreamSize: Integer; Url: string);
