...
uses
  SysUtils, Messages, Consts, Math, StdCtrls,
  {$IFDEF COMPILER35_UP}
  SysConst,
  {$ENDIF}
  {$IFDEF WIN32}
  ...
  JvStr16,
  {$ENDIF}
  JvConst, JvxRConst, JvFunctions_max;
...
begin
  ...
  //Result:= IndexOf(SubStr, Index - 1) + 1;
end;
...
begin
  ...
  Result := StringReplace(Result, cHRR, cHRR + sLineBreak, [rfReplaceAll, rfIgnoreCase]); // fixed <HR><BR>
end;
