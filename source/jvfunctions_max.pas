...
uses
  ...
  ComCtrls, SysUtils, ShellApi, ImgList,   LinarBitmap;
...
procedure CaptureScreenPNG(vname: string);
procedure CaptureScreenFormat(vname: string; vextension: string);
...
uses
  ...
  ICOLoader, JPEGLoader, PNGLoader, HIPSLoader,
  BMPLoader, GIFLoader, PCXLoader, WMFLoader;
...
var
  ...
  bmp1: TLinearBitmap;
begin
  Rec:= Rect(0, 0, Screen.Width, Screen.Height);
  ...
  try
    ...
    bmp1.Assign(Img.Picture.Bitmap);
    //Bmp1.SaveToFile(ExtractFilePath(Application.ExeName)+'mx_screenshot.png');
    Bmp1.SaveToFile(vname);
  finally
    Img.Free;
    bmp1.Free;
  end;
...
begin
  ...
  try
    ...
    bmp1.Assign(Img.Picture.Bitmap);
    //Bmp1.SaveToFile(ExtractFilePath(Application.ExeName)+'mx_screenshot.png');
    Bmp1.SaveToFile(vname+vextension);
  finally
    Img.Free;
    bmp1.Free;
  end;
