...
procedure Test(const Filename: String;
  ...
  begin
    if PathDrivePartLengthEx(Filename, False) <> DrivePartFalse then
      XRaise(Exception.CreateFmt('"%s" drive part(False) test failed',[Filename]));
    ...
    if PathPathPartLength(Filename, False) <> PathPartFalse then
      XRaise(Exception.CreateFmt('"%s" path part(False) test failed',[Filename]));
    if PathPathPartLength(Filename, True) <> PathPartTrue then
      XRaise(Exception.CreateFmt('"%s" path part(True) test failed',[Filename]));

    if PathIsRooted(Filename) <> (PathDrivePartLengthEx(Filename, True) <> 0) then
  {raise} XRaise(Exception.CreateFmt('"%s" PathIsRooted test failed',[Filename]));
  end;
  ...
  begin
    ...
    if inRemoveBackslash(Filename) <> ExpectedResult then
      XRaise(Exception.Create('RemoveBackslash test failed'));
  end;
  ...
  begin
    if RemoveBackslashUnlessRoot(Filename) <> ExpectedResult then
      XRaise(Exception.Create('RemoveBackslashUnlessRoot test failed'));
  end;
  ...
  begin
    if PathChangeExt(Filename, Extension) <> ExpectedResult then
      XRaise(Exception.Create('PathChangeExt test failed'));
  end;
  ...
  begin
    if inPathCombine(Dir, Filename) <> ExpectedResult then
      XRaise(Exception.Create('PathCombine test failed'));
  end;
  ...
  begin
    if PathStartsWith(S, AStartsWith) <> ExpectedResult then
      XRaise(Exception.Create('PathStartsWith test failed'));
  end;
...
begin
  ...
  if AlsoTestJapaneseDBCS then
    raise XRaise(Exception.Create('DBCS tests not supported in Unicode build');
  {$ENDIF}
  //if AlsoTestJapaneseDBCS and (GetACP <> 932) then
    //raise XRaise(Exception.Create('Must be running in Japanese code page to run these tests');
