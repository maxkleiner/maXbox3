...
//#sign:max: MAXBOX8: 11/25/2014 9:54:26 AM 

interface

type
  TNeuron = Tclass; //forward;//record{class} forward;
//------------------------------------------------------------------------------
  TSynapse = record
    W           : Real;
    Connection  : TNeuron;
...
//------------------------------------------------------------------------------
  ...
  //public
    procedure{constructor} CreateNL( ACountNeurons : Integer );
    {destructor} procedure  DestroyNL(); //override;
...
//------------------------------------------------------------------------------
  ...
  //public
    {constructor} procedure CreateNN();
    {destructor}  procedure DestroyNN(); //override;
  ...
  var
    ...
    {property} Inputs{[ Index  : Integer ]} : array of Real; //(( read GetInputs write SetInputs;
    {property} Outputs{[ Index : Integer ]} : array of Real; // read GetOutputs;
    {property} Results{[ Index : Integer ]} : array of Real; // read GetResults;
    {property} Epoch  : Integer; // read FEpoch;
    {property} awError  : Real; //    read FError;
...
procedure {constructor} {TNeuronLayer.}CreateNL( ACountNeurons : Integer );
var
  I : Integer;
...
//------------------------------------------------------------------------------
{destructor} procedure {TNeuronLayer.}DestroyNL();
var
  I : Integer;
...
begin
  for I := 0 to CountSynapses - 1 do
    FSynapse[ I ].W := ( Random(1) - Random(1) ) / 30 + 0.05;
...
begin
  ...
  FSynapse[ CountSynapses - 1 ].W := Random(1);
end;
//------------------------------------------------------------------------------
procedure {TNeuron.}ComputeW();
var
  I  : Integer;
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
procedure {constructor} {TNeuralNet.}CreateNN();
begin
  //inherited Create();
  FErrMinimal:=0.001;
...
procedure {destructor TNeuralNet.}DestroyNN();
var
  I : Integer;
...
//------------------------------------------------------------------------------
procedure {TNeuralNet.}ComputeW2();
var
 I : Integer;
...
begin
  ...
  //FLayers[ CountLayers - 1 ] := {TNeuronLayer.}CreateNL( ANumNeurons );
end; 
//------------------------------------------------------------------------------
