//
// (C) 06, Toolbox-Verlag und Wolfgang Soltendick, alle Rechte vorbehalten
//
...
begin
  ...
  // Berechnung
  result:= 60*1.852/Pi*180*arccos((sin(Phi1)*sin(Phi2))+(cos(Phi1)*cos(Phi2)*cos(Lambda1-Lambda2)));
...
begin
  ...
  Temp := int (Temp);
  Temp := Temp / 1000;
  Form1.EntfernungText.Caption := floattostr (Temp);
end;
...
begin
  ...
  Temp := int (Temp);
  Temp := Temp / 1000;
  Form1.Kurs1Text.Caption := floattostr (Temp);
  ...
  Temp := int (Temp);
  Temp := Temp / 1000;
  Form1.Kurs2Text.Caption := floattostr (Temp);
end;
