...
procedure Test(const Filename: String;
  ...
  begin
    if PathDrivePartLengthEx(Filename, False) <> DrivePartFalse then
      XRTLRaiseError(Exception.CreateFmt('"%s" drive part(False) test failed',[Filename]));
    if PathDrivePartLengthEx(Filename, True) <> DrivePartTrue then
      Exception.CreateFmt('"%s" drive part(True) test failed', [Filename]);
    if PathPathPartLength(Filename, False) <> PathPartFalse then
      XRTLRaiseError(Exception.CreateFmt('"%s" path part(False) test failed',[Filename]));
    if PathPathPartLength(Filename, True) <> PathPartTrue then
      XRTLRaiseError(Exception.CreateFmt('"%s" path part(True) test failed',[Filename]));

    if PathIsRooted(Filename) <> (PathDrivePartLengthEx(Filename, True) <> 0) then
  {raise} XRTLRaiseError(Exception.CreateFmt('"%s" PathIsRooted test failed',[Filename]));
  end;

  procedure TestRemoveBackslash(const Filename, ExpectedResult: String);
  begin
    //there are two removeBackslash!
    if inRemoveBackslash(Filename) <> ExpectedResult then
      XRTLRaiseError(Exception.Create('RemoveBackslash test failed'));
  end;
  ...
  begin
    if RemoveBackslashUnlessRoot(Filename) <> ExpectedResult then
      XRTLRaiseError(Exception.Create('RemoveBackslashUnlessRoot test failed'));
  end;
  ...
  begin
    if PathChangeExt(Filename, Extension) <> ExpectedResult then
      XRTLRaiseError(Exception.Create('PathChangeExt test failed'));
  end;

  procedure TestPathExtractExt(const Filename, ExpectedResult: String);
  begin
    if PathExtractExt(Filename) <> ExpectedResult then
      XRTLRaiseError(Exception.Create('PathExtractExt test failed'));
  end;

  procedure TestPathCombine(const Dir, Filename, ExpectedResult: String);
  begin
    if inPathCombine(Dir, Filename) <> ExpectedResult then
      XRTLRaiseError(Exception.Create('PathCombine test failed'));
  end;
  ...
  begin
    if PathStartsWith(S, AStartsWith) <> ExpectedResult then
      XRTLRaiseError(Exception.Create('PathStartsWith test failed'));
  end;
...
begin
  ...
  if AlsoTestJapaneseDBCS then
    raise XRTLRaiseError(Exception.Create('DBCS tests not supported in Unicode build');
  {$ENDIF}
  //if AlsoTestJapaneseDBCS and (GetACP <> 932) then
    //raise XRTLRaiseError(Exception.Create('Must be running in Japanese code page to run these tests');
  ...
  var part1, part2, expectr: string;
  //E: Exception;

begin
  PathFuncRunTests(false)
  ShowmessageBig2('OK.  all tests of Path  Func Run Tests passed!',false);
  
  //regress test
  part1:= 'c:\a.tar.gz';
  part2:= '.txt';
  expectr:= 'c:\a.tar.txt';
  writeln('PathChangeExt result: '+PathChangeExt(part1, part2));
  
   TestPathChangeExt('c:\a.tar.gz', '.txt', 'c:\a.tar.txt');
 
  expectr:= 'c:\a.tar.txt';
 
    if PathChangeExt(part1, part2) <> expectr then begin
      writeln(' not mot')
      //TIFException
      //E.Create('PathChangeExt test failed');
      //raiselastwin32;
      //E:= Exception.Create('PathChangeExt test failed');
      XRTLRaiseError(Exception.Create('PathChangeExt test failed'));
            //showException(Exception.Create('PathChangeExt test failed'),0);
      //XRTLRaiseError(Exception);
      //dumpexception;
      //raiselastexception;
      //raiseexception2;
      //RaiseException(ErTypeMismatch{erInvalidType},'Param: String');
    end;
      //raiselastexception
      //raiseexception2;
      //RaiseLastOSError
      //dumpexception;
      //checkException
      writeln(inRemoveBackslash('c:\\\'));
      writeln(RemoveBackslash('c:\\\'));
End.
