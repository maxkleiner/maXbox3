...
interface
  //uses Windows, Messages, Stdctrls, Sysutils, Classes, Grids;
        ...
        procedure Sortgrid1(Grid : TStringGrid; Const SortCol:integer);  //overload;
        procedure Sortgrid2(Grid : TStringGrid;
        ...
        procedure FreeAndClear1(C:TMemo);   //overload;
        procedure FreeAndClear2(C:TStringList);   //overload;
...
begin
  ...
  aCRLF:=chr(13) + chr(10);  {CR=#13=carriage retutn, LF=10=Linefeed}
  aCRCR:=chr(13)+chr(13);
 {temporarily change real paragraphs (blank line), CRLFCRLF to double CR}
  s:=stringreplace(s,aCRLF+aCRLF,aCRCR,[RfReplaceall]);
  {Eliminate input word wrap CRLFs}
  s:=stringreplace(s,aCRLF,' ',[RfReplaceall]);
  {now change CRCR back to CRLFCRLF}
  s:=stringreplace(s,aCRCR,aCRLF+aCRLF,[RfReplaceall]);
  m.text:=s;
  if m is TMemo then TMemo(m).wordwrap:=true; {make sure that word wrap is on}
end;

const EM_SETRECT = $403; //&H403
...
begin
  ...
  acr:=m.clientrect;
  if L>=0 then acr.left:=L;
  If T>=0 then acr.top:=T;
  If R>=0 then acr.right:=acr.right-r;
  If B>=0 then acr.bottom:=acr.Bottom-b;
  m.perform(EM_SETRECT,0,longint(___pointer(acr)));
...
Const EM_LINEFROMCHAR = $400 + 25; //&H400 + 25
...
//http://support.microsoft.com/kb/141073/de
Const EM_LINEINDEX = $400 + 11; //&H400 + 11
...
begin
  with memo, memo.lines do
  begin
    beginupdate;
    for i:=0 to count-2 do
    for j:= i+1 to count-1 do
    if AnsiCompareStr(lines[i],lines[j])>0 then begin  {swap(lines[i],lines[j])}
      s:=lines[i];
      lines[i]:=lines[j];
      lines[j]:=s;
    end;
...
{*********** SortGrid ************}
procedure Sortgrid2(Grid:TStringGrid; Const SortCol:integer; sortascending:boolean);
var
   i,j : integer;
   temp:tstringlist;
begin
...
{*********** SortGrid ************}
procedure Sortgrid1(Grid : TStringGrid; Const SortCol:integer);
begin
  Sortgrid2(grid,Sortcol, true);  {ascending}
 end;
